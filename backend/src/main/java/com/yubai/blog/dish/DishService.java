...
public class DishService {
    ...
    public PageResponse<DishResponse> findPublished(
            int page, int size, String categorySlug, String query) {
        ...
        if (hasQuery) {
            ...
            if (categoryName != null) {
                return PageResponse.from(
                        repository
                                .searchPublishedByCategory(categoryName, likeQuery, pageable)
                                .map(DishResponse::from));
            }
            return PageResponse.from(
                    repository
                            .searchPublishedEntities(likeQuery, pageable)
                            .map(DishResponse::from));
        }
        ...
        if (categoryName != null) {
            return PageResponse.from(
                    repository
                            .findByCategoryAndPublishedTrueOrderByFeaturedDescDisplayOrderAsc(
        ...
        return PageResponse.from(
                repository
                        .findAllByPublishedTrueOrderByFeaturedDescDisplayOrderAsc(pageable)
                        .map(DishResponse::from));
    }
    ...
    /** P0-7（已批准）：语义改为纯计数 favorite——每次调用 +1，不再假装 toggle。 P0-4：计数走数据库端原子 UPDATE。 */
    @Transactional
    ...
    public PageResponse<DishFavoriteItem> findFavorites(int page, int size) {
        return PageResponse.from(
                repository
                        .findAllByPublishedTrueOrderByFavoriteCountDesc(PageRequests.of(page, size))
    ...
    public PageResponse<DishResponse> findAll(int page, int size) {
        return PageResponse.from(
                repository
                        .findAllByOrderByDisplayOrderAsc(PageRequests.of(page, size))
                        .map(DishResponse::from));
    }
    ...
    @Transactional
    @CacheEvict(
            cacheNames = {CacheConfig.GRAPH, CacheConfig.SITEMAP},
            allEntries = true)
    public DishResponse create(DishRequest request) {
        ...
        return DishResponse.from(repository.save(DishEntity.create(request, slug)));
    }
                    ...
                    new TransactionSynchronization() {
                        ...
                        public void afterCommit() {
                            try {
                                ...
                                log.warn(
                                        "Failed to delete dish asset storage {}: {}",
                                        storageKey,
                                        e.toString());
                            }
                        }
    ...
    private String uniqueSlug(String name) {
        var base = DishSlug.fromName(name);
        var candidate = base;
        var suffix = 2;
        while (repository.existsBySlug(candidate)) {
            candidate = DishSlug.withSuffix(base, suffix++);
        }
        return candidate;
    }
}
