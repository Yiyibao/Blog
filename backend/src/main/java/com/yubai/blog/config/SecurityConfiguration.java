...
public class SecurityConfiguration {
    ...
    SecurityFilterChain securityFilterChain(
            ...
            com.yubai.blog.auth.SessionValidityFilter sessionValidityFilter)
            throws Exception {
        return http
                ...
                .authorizeHttpRequests(
                        auth ->
                                auth
                                        ...
                                        .hasAuthority(Permissions.METRICS_VIEW)
                                        .requestMatchers(
                                                "/actuator/health",
                                                "/actuator/info",
                                                "/api/v1/auth/login",
                                                ...
                                                "/error")
                                        .permitAll()
                                        // P2-3：文档路径放行但功能默认关闭（SPRINGDOC_ENABLED=false 时如实
                                        // 404），生产不暴露内容
                                        .requestMatchers(
                                                "/v3/api-docs/**",
                                                "/swagger-ui/**",
                                                "/swagger-ui.html")
                                        .permitAll()
                                        // L-16/D-17：/notes 与 /note-assets 移出公开白名单——学习笔记对游客真隐藏（落入下方
                                        // /api/** authenticated）
                                        .requestMatchers(
                                                HttpMethod.GET,
                                                "/api/v1/dish-assets/**",
                                        ...
                                        .hasAuthority(Permissions.KITCHEN_ACCESS)
                                        .requestMatchers("/api/v1/ai/**")
                                        .hasAuthority(Permissions.AI_USAGE)
                                        .requestMatchers("/api/v1/admin/ai/providers/**")
                                        .hasAuthority(Permissions.AI_MANAGE)
                                        .requestMatchers("/api/v1/admin/ai/**")
                                        .hasAuthority(Permissions.AI_USAGE)
                                        .requestMatchers("/api/v1/admin/stats")
                                        .hasAuthority(Permissions.DASHBOARD_VIEW)
                                        .requestMatchers(
                                                "/api/v1/admin/attachments/**",
                                                "/api/v1/admin/notes/*/attachments/**")
                                        ...
                                        .requestMatchers("/api/**")
                                        .hasAuthority(Permissions.ACCOUNT_ACCESS)
    ...
    @Bean
    com.yubai.blog.auth.SessionValidityFilter sessionValidityFilter(
            com.yubai.blog.auth.AdminUserRepository repository) {
        return new com.yubai.blog.auth.SessionValidityFilter(repository);
    }

    @Bean
    org.springframework.boot.web.servlet.FilterRegistrationBean<
                    com.yubai.blog.auth.SessionValidityFilter>
            sessionValidityFilterRegistration(com.yubai.blog.auth.SessionValidityFilter filter) {
        var registration =
                ...
                jwt -> {
                    ...
                    return roleClaim.stream()
                            .flatMap(
                                    roleName -> {
                                        try {
                                            return RolePermissions.forRole(
                                                    AdminUserRole.valueOf(roleName))
                                                    .stream();
