...
/**
 * FD-10：kitchen 包内业务异常映射（独立小 advice，不并入 common 大文件）；
 * 错误信封与全局同构 {status, message, timestamp}。
...
public class KitchenExceptionHandlers {
    ...
    @ExceptionHandler({MenuVersionConflictException.class, ObjectOptimisticLockingFailureException.class})
    @ResponseStatus(HttpStatus.CONFLICT)
    public Map<String, Object> handleVersionConflict(Exception exception) {
        var message = exception instanceof MenuVersionConflictException
    ...
    public Map<String, Object> handleBadRequest(KitchenBadRequestException exception) {
        return Map.of("status", 400, "message", exception.getMessage(), "timestamp", Instant.now().toString());
    }
}
