...
/**
 * 4C：文章版本历史——管理端每次保存快照一版（由 AdminPostController 编排调用，
 * 与 4B 删除钩子同法避免 PostService 构造器涟漪），按篇保留最近 {@link #KEEP} 版；
 * 恢复 = 把选中版本的正文字段回写文章并再快照一版（恢复本身也是一次保存）。
 */
@Service
...
public class PostRevisionService {
    ...
    public record RevisionSummary(long id, String title, ContentFormat contentFormat, Instant createdAt) {}

    public record RevisionDetail(long id, String title, String excerpt, String content,
                                 String markdownContent, ContentFormat contentFormat, Instant createdAt) {}

    private final PostRevisionRepository revisionRepository;
    private final PostRepository postRepository;

    public PostRevisionService(PostRevisionRepository revisionRepository, PostRepository postRepository) {
    ...
    public List<RevisionSummary> list(long postId) {
        ...
        return revisionRepository.findSummaries(postId).stream()
            .map(row -> new RevisionSummary(row.getId(), row.getTitle(), row.getContentFormat(), row.getCreatedAt()))
            .toList();
    }

    public RevisionDetail findOne(long postId, long revisionId) {
        requirePost(postId);
        var revision = revisionRepository.findByIdAndPostId(revisionId, postId)
    ...
    public PostResponse restore(long postId, long revisionId) {
        ...
        var revision = revisionRepository.findByIdAndPostId(revisionId, postId)
            .orElseThrow(() -> new NotFoundException("版本不存在：" + revisionId));
        post.applyRevision(revision.getTitle(), revision.getExcerpt(), revision.getContent(),
            revision.getMarkdownContent(), revision.getContentFormat());
        revisionRepository.save(PostRevisionEntity.snapshot(post));
        var all = revisionRepository.findAllByPostIdOrderByCreatedAtDescIdDesc(postId);
        if (all.size() > KEEP) {
            revisionRepository.deleteAll(all.subList(KEEP, all.size()));
    ...
    private static RevisionDetail toDetail(PostRevisionEntity revision) {
        return new RevisionDetail(revision.getId(), revision.getTitle(), revision.getExcerpt(),
            revision.getContent(), revision.getMarkdownContent(), revision.getContentFormat(),
