...
/**
 * 4A-1：聊天编排——解析供应商端点（注册表优先、env 回退）后调用对应类型客户端。 OPENAI_COMPATIBLE → OpenAiCompatibleClient；ANTHROPIC
 * → AnthropicClient；OPENCODE_SERVER → OpenCodeServerClient。
 */
@Service
public class AiChatService {
    ...
    private final AiCallReliabilityPolicy reliabilityPolicy;
    ...
    public AiChatService(
            ...
            AiCallReliabilityPolicy reliabilityPolicy) {
    ...
    public ChatResponse chat(ChatRequest request) {
        ...
        return audited(
                ...
                request,
                () ->
                        reliabilityPolicy.execute(
                                endpoint,
                                true,
                                () ->
                                        reasoningEffort == null
                                                ? clientFor(endpoint)
                                                        .chat(endpoint, request.messages())
                                                : clientFor(endpoint)
                                                        .chat(
                                                                endpoint,
                                                                request.messages(),
        ...
        return audited(
                ...
                () ->
                        reliabilityPolicy.execute(
                                ...
                                () ->
                                        reasoningEffort == null
                                                ? clientFor(endpoint).stream(
                                                        endpoint, request.messages(), listener)
                                                : clientFor(endpoint).stream(
                                                        endpoint,
                                                        request.messages(),
                                                        listener,
    ...
    private ChatResponse audited(
            AiEndpoint endpoint,
            ChatRequest request,
            java.util.function.Supplier<ChatResponse> call) {
        var estimatedInputTokens =
                Math.max(
                        ...
                        request.messages().stream()
                                        ...
                                        .sum()
                                / 4);
        usageService.assertWithinDailyBudget(endpoint, estimatedInputTokens);
    ...
    /**
     * 限额校验的唯一入口（条数/单条长度/总长）。控制器在建立 SSE 流之前调用以便返回普通 HTTP 400；chat/stream 内部同样执行，保证绕过控制器的调用方也不会失守。
     */
    public void validateLimits(ChatRequest request) {
        ...
        if (request.messages().stream()
                .anyMatch(m -> m.content().length() > properties.getMaxInputChars())) {
            throw new AiServiceException(
                    HttpStatus.BAD_REQUEST,
    ...
    private String resolveReasoningEffort(ChatRequest request, AiEndpoint endpoint) {
        ...
        if (endpoint.providerType() == AiProviderType.OPENCODE_SERVER) {
            throw new AiServiceException(HttpStatus.BAD_REQUEST, "当前供应商不支持推理强度选择");
        }
        if (endpoint.providerType() == AiProviderType.OPENAI_COMPATIBLE
                && OpenAiCompatibleClient.isDeepSeekEndpoint(endpoint.baseUrl())
                && !"none".equals(effort)) {
            throw new AiServiceException(HttpStatus.BAD_REQUEST, "DeepSeek 兼容端点仅支持推理开关，不支持强度等级");
        }
        return effort;
    }
}
