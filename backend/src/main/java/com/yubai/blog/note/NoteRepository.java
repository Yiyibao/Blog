...
public interface NoteRepository extends JpaRepository<NoteEntity, Long> {
    ...
    @Query(
            "SELECT n.id as id, n.updatedAt as updatedAt FROM NoteEntity n WHERE n.status = com.yubai.blog.note.NoteStatus.PUBLISHED")
    ...
    @Query(
            "SELECT n.id as id, n.title as title, n.markdownContent as markdownContent FROM NoteEntity n")
    List<NoteRefRow> findAllRefRows();

    /** 3C：浏览量数据库端原子自增（IP+id 短窗去重在 Controller 层，不落 IP 明文；仅已发布计数）。 */
    @org.springframework.data.jpa.repository.Modifying
    @Query(
            "UPDATE NoteEntity n SET n.viewsCount = n.viewsCount + 1 WHERE n.id = :id AND n.status = com.yubai.blog.note.NoteStatus.PUBLISHED")
    ...
    /** NB-5：图谱节点行（不载正文）。 */
    @Query(
            "SELECT n.id as id, n.title as title, n.folder as folder, n.updatedAt as updatedAt FROM NoteEntity n WHERE n.status = com.yubai.blog.note.NoteStatus.PUBLISHED")
    List<NoteGraphRow> findPublishedGraphRows();

    /** NB-5：图谱标签边（[noteId, tag]，仅已发布）。 */
    @Query(
            "SELECT n.id, t FROM NoteEntity n JOIN n.tags t WHERE n.status = com.yubai.blog.note.NoteStatus.PUBLISHED ORDER BY n.id")
    ...
    @Query(
        ...
        LEFT JOIN n.tags tag
        WHERE n.status = com.yubai.blog.note.NoteStatus.PUBLISHED
          AND n.updatedAt >= :fromDate AND n.updatedAt < :toDate
          AND (LOWER(n.title) LIKE LOWER(:query)
            OR LOWER(n.folder) LIKE LOWER(:query)
            OR LOWER(n.markdownContent) LIKE LOWER(:query)
            OR LOWER(tag) LIKE LOWER(:query))
        ...
        WHERE n.status = com.yubai.blog.note.NoteStatus.PUBLISHED
          ...
          AND (LOWER(n.title) LIKE LOWER(:query)
            ...
            OR LOWER(tag) LIKE LOWER(:query))
        """)
    Page<NoteSearchRow> searchPublishedBetween(
