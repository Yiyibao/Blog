...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<Map<String, Object>> handleInvalidFile(
            InvalidNoteFileException exception) {
        return error(HttpStatus.BAD_REQUEST, exception.getMessage());
    }

    @ExceptionHandler(com.yubai.blog.dish.InvalidRecipeException.class)
    public ResponseEntity<Map<String, Object>> handleInvalidRecipe(
            com.yubai.blog.dish.InvalidRecipeException exception) {
        return error(HttpStatus.BAD_REQUEST, exception.getMessage());
    }

    @ExceptionHandler(NoteVersionConflictException.class)
    public ResponseEntity<Map<String, Object>> handleNoteConflict(
    ...
    public ResponseEntity<Map<String, Object>> handleTooManyRequests(
        ...
        return ResponseEntity.status(HttpStatus.TOO_MANY_REQUESTS)
                ...
                .body(errorBody(HttpStatus.TOO_MANY_REQUESTS, exception.getMessage()));
    }

    /** L-7：人机验证失败统一 400，文案不区分具体原因。 */
    @ExceptionHandler(ChallengeVerificationException.class)
    public ResponseEntity<Map<String, Object>> handleChallengeVerification(
            ChallengeVerificationException exception) {
        return error(HttpStatus.BAD_REQUEST, exception.getMessage());
    }

    /** L-7：登录冷却 429 附带 Retry-After，便于客户端与上游按标准语义处理。 */
    @ExceptionHandler(LoginCooldownException.class)
    public ResponseEntity<Map<String, Object>> handleLoginCooldown(
            LoginCooldownException exception) {
        return ResponseEntity.status(HttpStatus.TOO_MANY_REQUESTS)
                .header(
                        "Retry-After",
