...
class AdminAiImageControllerTest {
    ...
    void sessionsReturnsList() throws Exception {
        when(service.listSessions("admin"))
            .thenReturn(List.of(new AiImageSessionResponse(1L, "雨后的西湖", Instant.now(), Instant.now())));
    ...
    void deleteSessionDelegates() throws Exception {
        mockMvc.perform(delete("/api/v1/admin/ai/images/sessions/1"))
            .andExpect(status().isOk());
    ...
    void generateReturnsSessionAndImages() throws Exception {
        ...
        var image = new AiGeneratedImageResponse(
            publicId, UUID.randomUUID(), "grok", "grok-imagine-image-quality", "a cat in the rain",
            "image/png", 1234L, 1024, 1024, "/content/url", Instant.parse("2026-08-04T01:00:00Z"));
        when(rateLimiter.tryAcquire(any(String.class), any(Integer.class), any()))
            .thenReturn(true);
        ...
        mockMvc.perform(post("/api/v1/admin/ai/images")
                .contentType(MediaType.APPLICATION_JSON)
                .content("{\"prompt\":\"a cat in the rain\",\"sessionId\":7,\"provider\":\"grok\"}"))
            .andExpect(status().isOk())
