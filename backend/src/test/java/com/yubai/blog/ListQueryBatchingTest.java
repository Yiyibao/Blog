 ...
 *
 * <p>文章/笔记列表自 L-12 起走 Service 层「投影行 + 一次标签 IN 批查询」， prepare 次数必须 ≤2 且不触正文列；菜谱列表仍为实体路径，@BatchSize
 * 批量抓取 后 prepare 次数必须 ≤3（1 条主查询 + ingredients/steps 各 1 条 IN 批量查询； page size 大于总行数时无 count 查询）。
 */
...
class ListQueryBatchingTest {
    ...
    @Autowired com.yubai.blog.kitchen.MealLogRepository mealLogRepository;

    @Autowired com.yubai.blog.kitchen.DailyMenuRepository dailyMenuRepository;

    @Autowired com.yubai.blog.kitchen.DailyMenuItemRepository dailyMenuItemRepository;
    ...
    void postListLoadsTagsInBatches() {
        ...
        long prepares =
                measure(
                        () ->
                                service.findPublished(0, 50, null, null)
                                        .items()
    ...
    void noteListLoadsTagsInBatches() {
        ...
        long prepares =
                measure(
                        () ->
                                service.findAll(NoteStatus.DRAFT, 0, 50)
                                        .items()
    ...
    void dishListLoadsIngredientsAndStepsInBatches() {
        ...
        long prepares =
                measure(
                        () ->
                                dishRepository
                                        .findAllByPublishedTrueOrderByFeaturedDescDisplayOrderAsc(
                                                PageRequest.of(0, 50))
                                        .forEach(
                                                dish -> {
    ...
    void mealLogTimelineDoesNotFetchDishCollections() {
        ...
        for (int i = 1; i <= SEED; i++) {
            mealLogRepository.save(
                    com.yubai.blog.kitchen.MealLogEntity.create(
                            LocalDate.of(2026, 2, i),
                            dish.getId(),
                            "打卡防线菜",
                            com.yubai.blog.kitchen.MealSlot.DINNER,
                            null,
                            "",
                            1L,
                            "站长"));
        ...
        var service =
                new com.yubai.blog.kitchen.MealLogService(
                        mealLogRepository,
                        dailyMenuRepository,
                ...
                measure(
                        () ->
                                service.timeline(0, 50, null, null, null).items().stream()
                                        ...
                                        .forEach(
                                                log ->
                                                        assertThat(log.dishSlug())
                                                                .isEqualTo("qc-meallog-dish")));
