...
> 当前统一入口：先生成并验证备份文件，再运行 `scripts/migration-preflight.ps1`。脚本会拒绝缺失或空备份，并输出当前 Flyway 版本与内容行数。开发演示内容只能显式运行 `scripts/dev-seed.ps1`；生产启动与后续 Flyway 迁移禁止写入演示数据。V34/V39 作为已执行历史事实保持只读。
