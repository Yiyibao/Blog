...
8. AI platform: authenticated site owners use persistent sessions/tasks/events, controlled file uploads, confirmed memories and controlled artifacts through `/api/v1/ai/**`. The legacy `/api/v1/admin/ai/**` chat and image APIs remain compatible.

## AI multimodal platform (M1 Internal Alpha)

The M1 AI core lives in `com.yubai.blog.ai`; existing provider clients remain in
`com.yubai.blog.admin.ai` behind `AiModelGateway`. PostgreSQL is authoritative for sessions,
tasks, structured parts, events, memories, file metadata and artifact metadata. `StorageService`
is authoritative for uploaded and generated bytes. Browser storage is never the fact source.

### Request and persistence flow

1. The authenticated user uploads a supported file. `AiFileService` enforces owner count/byte
   quotas, bounded reads, extension/MIME/magic validation and format-specific structural limits.
2. The client creates a persistent task with `TEXT`, `IMAGE_REF` and/or `FILE_REF` parts. File and
   artifact inputs are references only; client or server paths are not accepted.
3. `AiModelGateway` resolves a provider and checks an explicit protocol capability matrix before
   reading file bytes. The M1 OpenAI Responses adapter serializes `input_text`, `input_image` and
   `input_file` parts with `store=false`; unsupported providers fail explicitly.
4. Task events receive a per-task database sequence and can be replayed with `afterSequence` or
   `Last-Event-ID`. Optimistic task versions enforce one terminal state; stale running tasks become
   explicit `INTERRUPTED_BY_RESTART` failures at startup.
5. Before every run, `AiContextWindowService` reconstructs context from PostgreSQL in a stable
   order: confirmed owner/scope memories, the server-side summary of older turns, recent completed
   user/assistant text, then the current task and uploads. OpenAI Responses receives distinct
   `user`/`assistant` messages; legacy providers receive the same role sequence instead of one
   flattened prompt.
6. Only user-created memory, or a task-sourced proposal explicitly confirmed by the user, becomes
   `ACTIVE`. `USER`/`GLOBAL`/`SITE` scopes cross sessions; `SESSION:{id}` only enters its owned
   session. Disable, expiry and forget take effect on the next run. Forget clears the body and
   owner session summaries so no derived summary cache keeps the deleted content.
7. Memory, summary and recent-message layers have independent configurable token estimates. If a
   layer is trimmed, the task records `context.truncated` with per-layer flags. Older completed
   turns are compacted into `ai_sessions.summary`, which is visible and clearable by the owner.
8. Markdown, TXT, JSON, CSV and existing generated images are copied to controlled artifact
   storage with owner, task, SHA-256, byte size and expiry metadata. Downloads are authenticated
   attachments with `private, no-store` and `nosniff`; CSV cells are protected from formula
   injection.

### Supported M1 uploads

| Family | Types | Key limits |
| ------ | ----- | ---------- |
| Images | PNG, JPEG, WebP | MIME/magic and decoded dimensions |
| Documents | PDF, DOCX | PDF magic/page/encryption checks; DOCX ZIP entry, path, macro and expanded-byte checks |
| Text/data | TXT, Markdown, CSV, JSON | strict UTF-8, extracted characters, CSV rows/columns, JSON depth/string size |

### Operations and compatibility

- Flyway `V53__create_ai_multimodal_platform.sql` is expand-only and preserves V46 chat history.
- `APP_AI_PLATFORM_TASKS_ENABLED`, `MULTIMODAL_ENABLED`, `MEMORY_ENABLED` and
  `ARTIFACTS_ENABLED` all default to `false`; the production UI defaults to the legacy chat.
- Global/per-user/per-provider running limits and a bounded queue reject overload with HTTP 429
  and `Retry-After`.
- Scheduled cleanup marks expired files/artifacts and deletes bytes after the metadata transaction
  commits; failed storage deletes are retried on the next run. Creation failures compensate bytes.
- AI image content/delete rechecks image-session ownership, closing the prior public-ID IDOR gap.
- The table/field reference is in `docs/ai-platform-data-dictionary.md`; design boundaries are in
  `docs/adr/ADR-001-ai-multimodal-platform.md`.

### M12 recovery, accessibility and capacity closeout

- The reviewed release path is nginx static frontend + one Spring Boot instance + PostgreSQL +
  controlled local storage. In-process login limits, auth cooldowns and AI bounded concurrency
  mean a second backend replica is not safe without shared state.
- A restore batch is a PostgreSQL custom dump plus storage archive, per-file inventory and a
  SHA-256 batch manifest. `scripts/restore-and-migrate-drill.ps1` restores both sides into an
  isolated cluster and verifies Flyway V62, content counts, lifecycle tables, deleted attachment
  metadata and the query audit.
- Search currently uses PostgreSQL content queries and `pg_trgm` indexes. There is no separate
  `ai_knowledge_documents`/embedding index in the implemented schema, so restore and forgetting
  do not claim an independent vector-index operation.
- Public/admin/AI/kitchen accessibility gates use Axe WCAG 2A/2AA severe-impact filtering plus a
  keyboard focus smoke. Manual screen-reader review and production telemetry remain release
  prerequisites, not facts inferred from local tests.
- Capacity triggers and the decision to retain the simple topology are recorded in
  `docs/adr/ADR-003-annual-architecture-capacity-gate.md`.
