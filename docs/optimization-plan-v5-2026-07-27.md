...
> 状态：**已批准（2026-07-27，用户确认；D-8~D-14 按默认建议，D-12 版本历史 N=10）**。本文取代 docs/optimization-plan-v4-2026-07-26.md 成为唯一执行依据。
>
> **开工前置（用户 2026-07-27 指令）**：批准后暂缓启动新条目——等待 L-7/4A 在途并行会话全部提交后，先执行一次**合并验证**（工作区归零 → 双端全量测试 → main 快进推送并留档），确认 main 仓库代码正确，之后方可按条目开工。
> **→ 已满足（2026-07-27）**：合并验证完成（docs/checkpoint-2026-07-27-merge-verify.md，后端 239/239 + 前端 187/187）；此后 4A-3、P2-6、package-lock 跨平台修复亦已并入，CI 自 dd97dd1 起全绿。
>
> **修订记录（2026-07-27 第二批）**：应用户要求新增 **L-13~L-16** 四个重点条目（归档图谱动态化重排、笔记编辑器 Typora 化二期、人机验证弹窗化、入口大屏与角色化路由），明细见 3.7；美食页面专项（FD-\*）由并行会话独立执行，本计划仅作台账衔接（见 1.2 与 3.3）。
>
...
**2026-07-27 增量（首版发布后已完成并合并 main，CI 全绿）**：

| 条目 | 提交 | 备注 |
| --- | --- | --- |
| L-7 登录人机验证 | 6136265 | 三层防御完整落地，24 后端 + 9 前端用例，验收自查见 checkpoint-2026-07-27-l7-ai |
| 4A-1/4A-2 收尾加固 | 90b36b2 | AI 模块对抗性 review 加固（SSE 心跳/中断止损/供应商兼容等），V16 唯一默认索引 |
| 4A-3 供应商管理 UI | ba607c2 | 检查点补档 5714097 |
| P2-6 npm audit 分层 | be8e91e | 生产依赖阻塞审计（0 漏洞）+ 完整审计非阻塞记录 |
| package-lock 跨平台修复 | dd97dd1 | Windows npm install 反向裁剪 lock 致 CI npm ci 挂，外科补条目零漂移；**本机禁用裸 npm install 更新 lock** |

**并行专项：美食页面优化与功能拓展（FD-\*，独立会话执行中）**——目标：更高级 UI/动画、今日菜单协作（与女友共同定制）、收藏榜前端接入、第二账号等。条目号 **FD-\***、方案/验收/排期以其专项文档与 checkpoint 为准，本计划不重复展开；其**预定占用迁移号 V17**（见 3.3）。耦合点：第二账号动鉴权体系，与 L-16 入口/角色化路由必须以 checkpoint 互认契约、authStore 单一事实源不得分叉。
...
| 阶段 | 主题 | 核心产出 | 估时 | 前置 |
| --- | --- | --- | --- | --- |
| 1（剩余） | 性能优化 | **✅ 已完成（2026-07-27，checkpoint-2026-07-27-stage1-perf）**：P1-3/4/5/6/8、NB-1/5、L-12、NF-6/7、P1-7、L-3；后端 271/271 + 前端 236/236；例外：NF-10 与 FD 第二批冲突顺延（收口后立即执行，仍属阶段 1 口径） | — | 已过验收门（Lighthouse 待用户手测留档） |
| 2 | 工程质量与交付流水线 | **✅ 基本完成（2026-07-27，checkpoint-2026-07-27-stage2-quality）**：P2-1~P2-5/P2-9/P2-10、NB-4/6/7/8/11、NF-4/8/11 与 NF-9(3/4)、L-8/L-9/L-11、CD 工作流、D-11（P2-6/P2-8 此前已完成）。例外：P2-7 renormalize 顺延至美食专项收口静默窗（连带 NF-9 移交项与 NF-10）；NB-9 需用户 flyway repair 决策，跳过挂起；NF-12 移交美食专项（V20 预留） | 6–8 天 | 阶段 1 |
| 2B | **✅ 已完成（2026-07-27，checkpoint-2026-07-27-stage2b-user-batch）** | L-13 图谱重排动态化（2fa4160）、L-15 验证弹窗（d58ebc6）、L-16 游客收权+入口大屏（1c9e8c9/8ec61f2）、L-14 代码高亮（646c919）；D-15~D-18 按默认建议执行；后端容器模式 333/333、前端 310/310 全绿 | 8–11 天 | 已过验收 |
| — | 美食页面专项（FD-\*） | 并行会话独立执行：高级 UI/动画、今日菜单协作、收藏榜接入、第二账号 | 以专项文档为准 | 独立推进；与 L-16 有鉴权耦合 |
| 3 | **✅ 已完成（2026-07-27，checkpoint-2026-07-27-stage3-markdown）** | 3A-1~4 全落地（4f1a455/81dc380）：V21 双字段、转换工具+校对清单、TyporaEditor 接管文章、受控渲染双防线；3B TOC 源文提取；3C 三类浏览量（V22，ea32aa0）；3D RSS+相邻导航全做。例外：3A-5 全局存量签收属用户环节（按篇编辑保存即签收，机制已内建），旧 HTML 列去留评审待签收后 | 2–3 周 | 已过验收（存量签收除外） |
| 4 | **✅ 已完成（2026-07-27，checkpoint-2026-07-27-stage4-admin）** | 4A-3~6（ba607c2/cee875c/9414fb2/c743738）、4F（69b899f）、4B（654e365）、4C（56a7e6c，V23）、4D+4E（e7a76b7，V24）；容器模式 368/368 + 前端 330/330 全绿。偏差记档：4C N=10 默认、4E 回收站简化为孤儿标记+确认删除、编排层挂点原则 | 3–4 周 | 已过验收 |
...
| P1-4 | 搜索 LIKE '%..%' 对 posts.content/notes.markdownContent 全表扫描（V9 注释自证 pg_trgm 未装） | SearchService、PostRepository | **新迁移（号以 3.3 台账与执行时实际最高号为准）**：`CREATE EXTENSION pg_trgm` + 三表搜索列 GIN 索引；查询保持 LIKE 即受益 | EXPLAIN (ANALYZE) 走索引，提交说明附前后对比；搜索测试全绿 | 0.5 天 |
...
| NF-12 | 菜谱份量基准硬编码 2 | 新迁移（号以 3.3 台账为准）加 dishes.base_servings，前端按其缩放；美食专项若顺带实现则本条销案 | 0.5 天 |
...
v4 3.6 的三层设计**已按图实现**（在途）：层 1 PoW 常开（难度 4、TTL 5 分钟、一次性、绑 IP、恒定时间比较）；层 2 失败≥3 图形码（Java2D 160×56、字符集剔除易混形、答案哈希存储、大小写不敏感）；层 3 失败≥10 冷却 30 分钟（429 + Retry-After）；配置化 app.auth.challenge.*（六键，env 可覆盖）；nginx challenge 15r/m 专属限速；零新依赖。收尾三步见 1.2 在途表（**已全部完成**，见 1.2 增量表与 checkpoint-2026-07-27-l7-ai）。验收 agent 按 v4 3.6「测试与验收」小节原文逐条核验，另加两条：challenge 端点在冷却期同样 429；risk 升级后旧 POW challenge 作废（已有单测，验收复核）。

### 3.7 用户新增重点批次（2026-07-27 · L-13~L-16，已批准执行，细节决策见 D-15~D-18）

**L-13 · 归档页知识图谱重排与动态化重设计（前端，2–3 天）**

- **布局重排**：移除「时间轴 / 关联图」视图切换 tab，知识图谱**常驻时间轴上方**、同页纵向呈现（图谱在上、时间轴在下）；按用户原话「不用分类」移除类型筛选分类 tab（是否连带节点关联筛选一起移除见 D-15）。
- **动态化**：现有确定性静态 SVG 布局 → **动态图谱**：节点入场动画（错峰浮现/连线绘制）、持续的轻微呼吸/漂浮态（非永久大幅循环）、力导向或带缓动的布局过渡。
- **交互动画**：hover 高亮节点及邻接边（其余淡出）、点击聚焦/展开邻居（衔接阶段 5 的 5C 子图端点，先做前端本地聚焦）、平移/缩放、节点拖拽（可选）。
- **约束继承**：`prefers-reduced-motion` 降级为静态布局（既有无障碍基线不弃守）；键盘可达不回归；帧预算——节点 >300 自动降级（与 5C 衔接）；默认零新依赖自绘（如需 d3-force 见 D-16）。
- 测试/验收：布局层级正确（图谱→时间轴单页流）；reduced-motion 降级有测试；hover/点击交互组件测试；旧 ?view=graph 链接的兼容语义明确。

**L-14 · 笔记编辑器 Typora 化二期：工具栏清晰化 + 代码块语言高亮（前端为主，2–3 天）**

- **工具栏**：H1/H2 等按钮的字形与字号更清晰（对比度、视觉层级、active/hover 态），整体贴近 Typora 的排版语感；工具栏项分组与间距重排。
- **代码块语言高亮**：插入代码块时**由作者标明语言**（代码块头部语言选择/输入，落库为 markdown 围栏语言标记）；按语言语法高亮。技术路线：`@tiptap/extension-code-block-lowlight` + `lowlight`（highlight.js 内核）——**本条目明确引入这两个 npm 依赖**（红线 3 的点名豁免）；按需注册常用语言集（js/ts/java/sql/bash/json/html/css/python 等）控制体积，随管理路由懒加载，公开首屏 chunk 不得增大（衔接 P1-7）。
- **渲染一致性**：公开笔记只读渲染（PublicNotes）同步高亮；3A-4 文章 Markdown 化落地后代码块走同一管线；高亮输出仍按不可信内容过既有消毒防线，publicNotesXss 测试扩展代码块用例。
- 验收：语言标记保存-重开往返一致（含导出 .md 围栏标记）；公开页高亮生效且 XSS 用例绿；工具栏可用性对照 Typora 走查。

**L-15 · 人机验证「点击式弹窗」形态（前端为主 + 复用既有后端，1–1.5 天）**

- **现状说明**（用户「没找到在哪」的原因）：L-7 三层验证已上线，但层 1 PoW 是无感设计——校验后台静默完成，界面只有「安全校验中」瞬态。本条目把验证**显性化**为独立弹窗交互。
- **形态**：进入登录流程先弹验证弹窗——点击「我不是机器人」按钮 → 弹窗内执行既有 challenge 流程（Web Worker 算 PoW，展示进度动画 → 通过打勾）；触发升级时**同弹窗内**展示图形码输入（换一张）；冷却时展示剩余时间（429 Retry-After）。
- **协议不变**：完全复用 GET /api/v1/auth/challenge 与登录携带 challengeId/nonce/captchaAnswer 的既有契约，后端零改动。
- 验收：三层状态（点击通过/图形码/冷却）弹窗内闭环且组件测试覆盖状态机；focus trap + aria-live；关闭弹窗=中止登录的语义明确。

**L-16 · 入口大屏与角色化路由（前端为主 + 后端配合，3–4 天）**

- **入口大屏**：访问站点根路径呈现全屏入口——复用现有首页 hero 大屏背景图视觉，中央两个选择：「以游客身份进入」/「以管理员身份进入」；入口视觉与过渡动效单独设计（开场感）。
- **游客路径**：进入首页，全站与现状一致但**学习笔记整体隐藏**（导航项、首页入口、/notes 路由对游客不可见不可达）。
- **管理员路径**：跳转登录页 → 先过 L-15 验证弹窗 → 登录成功进入首页（内容与现状一致）+ 顶部导航增加**「进入后台」按钮**。
- **关键设计决策**（默认建议见 D-17/D-18）：
  1. 笔记隐藏边界：仅前端隐藏（接口仍公开可直连）vs **后端同步收权**（/api/v1/notes、/api/v1/note-assets 移出白名单；归档时间轴、知识图谱、全局搜索、sitemap 的笔记条目对游客剔除）。真隐藏须选后者，属公开 API 破坏性变更并影响 SEO（笔记退出收录）。
  2. 深链行为：分享的文章/菜谱直链**不被入口屏拦截**——入口屏仅根路径且无既往选择时出现，选择记 localStorage；深链直达默认游客态。
  3. 鉴权耦合：角色判定统一走 authStore（游客=无 token；管理员=既有 JWT），与美食专项（FD）第二账号改造共用一套角色体系，双方以 checkpoint 互认契约，禁止两套并行逻辑。
- 测试/验收：三条路径（游客首访/管理员登录/深链直达）路由测试；游客态笔记不可达按 D-17 边界验证（选后端收权则加集成测试：游客访问 /api/v1/notes 401/403、sitemap 无笔记 URL）；管理员首页含后台按钮；入口选择记忆与「切换身份」重置路径明确。
...
- 继承 v4：D-1（CD 提前阶段 2）、D-2（读者增长降权）、D-3（L-1~L-6 纳入）、D-6（AI 配置 DB 注册表 + AES-GCM，已实现）、D-7（自托管三层人机验证，已实现在途）。
- **2026-07-27 用户批准本文全部新增决策**：D-8~D-11、D-13、D-14 按默认建议；D-12 版本历史保留 **N=10**。同时指令「批准后暂缓开工，待并行会话完成后先做合并验证」（见文首开工前置）。

### 7.2 本文新增、已随批准定案（保留原表备查）
...
| D-15 | L-13「不用分类」的边界：仅移除类型筛选 tab，还是连节点点击驱动的关联（relation）筛选一并移除 | 仅移除类型 tab，保留节点点击关联交互 |
| D-16 | L-13 动态布局是否引入 d3-force（约 15KB gzip）或继续零依赖自绘 | 零依赖自绘（简化力导向/缓动编排），不达效果再评 d3-force |
| D-17 | L-16 笔记隐藏边界：仅前端隐藏 vs 后端同步收权（/notes、/note-assets 出白名单 + 时间轴/图谱/搜索/sitemap 对游客剔除笔记） | **后端同步收权**（真隐藏；接受笔记退出 SEO 收录） |
| D-18 | L-16 入口屏出现时机：每次访问 vs 仅根路径首访 + localStorage 记忆 + 深链直达游客态 | 仅根路径首访 + 记忆 + 深链直达（分享链接不被拦截） |

D-1~D-14 已随 2026-07-27 首次批准定案；**D-15~D-18 为第二批（L-13~L-16）附带决策，待用户确认**——未确认前执行 agent 按默认建议推进，不阻塞开工。
