...
>
> **当前基线（2026-07-28）**：后端 460/460、前端 426/426、typecheck+build 全绿。
>
...
| 6D SEO 预渲染 | ✅ 已完成 | 构建期 HTML head 快照；静态公开页常驻，文章/合集/标签可选 API 批量生成 |
| 6E 备份演练 | ✅ 已完成 | Lighthouse 本地+COS 自动备份已部署；COS 往返哈希与隔离恢复通过，timer 已启用 |
| 6F 依赖审计 | ✅ 2026-07 月度审计 | npm 生产依赖 0 漏洞；12 个 high 均在测试/构建链；major 升级留独立评审 |
| 合集乐观锁响应 | ✅ 已修复 | 成员整表替换后先 flush，响应返回已递增版本；后端全量恢复 456/456 |
| 会话失效覆盖 | ✅ 已修复 | sessions_valid_from 覆盖密码/TOTP、私有笔记与附件；公开认证入口保持免查库 |
