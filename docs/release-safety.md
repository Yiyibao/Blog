...
| Flyway | 11.7.2（Spring Boot 3.5.16 BOM） | V1–V55 fresh、V39–V55、V52–V55、V53–V55 |
| PostgreSQL | 17 | CI/Testcontainers 的正式发布基线 |
| PostgreSQL | 18 | 本机兼容验证；升级生产前仍需独立演练 |
| Node.js | 22 | CI 与生产前端构建 |

`docs/migrations/flyway-content-manifest.sha256` 冻结 V1–V55 的 SHA-256。历史迁移不得修改；新增迁移必须同时追加清单并更新 `deploy/release-compatibility.env`。CI 运行 `node scripts/verify-migration-manifest.mjs`，文件集合、内容或目标版本漂移都会失败。

## Fresh、upgrade 与 rollback

- 真正空 schema 首次启动时执行全部历史迁移，然后仅对该 fresh 安装清除历史演示文章、菜品、分类、音乐和语录，最终保留 V55 空内容基线。
...
- V54–V55 为 expand-only：V53 的表和列全部保留，新引用可空或带默认值。代码可回滚到 schema 下限不低于 V53 的上一发行版，但数据库仍保持 V55；代码回滚不伪装成数据库回滚。
...
`scripts/restore-and-migrate-drill.ps1` 可恢复数据库与 storage archive，逐文件核对 inventory，并验证 V55、附件回收站、AI budget、AI lifecycle 表和知识索引元数据。数据库 metadata 与 shared storage 必须作为同一批次恢复，不能只恢复其中一侧。
