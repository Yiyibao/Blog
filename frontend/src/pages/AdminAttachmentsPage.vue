...
import {
  ...
  fetchAttachmentOverview,
  deleteNoteAttachment,
  type AttachmentOverview,
  type AttachmentOverviewItem,
} from '../api/admin';
      ...
      <template v-else-if="overview">
        <div class="attachment-summary">
          <div class="summary-card">
            <small>附件总数</small><strong>{{ overview.count }}</strong>
          </div>
          <div class="summary-card">
            <small>总容量</small><strong>{{ formatBytes(overview.totalBytes) }}</strong>
          </div>
        ...
        <table v-else class="attachment-table">
          ...
          <tbody>
            <tr v-for="item in items" :key="item.id" :class="{ orphan: item.orphan }">
              ...
              <td>
                <span v-if="item.orphan" class="orphan-badge" title="所属笔记正文不再引用且创建超 7 天"
                  >孤儿</span
                >
...
tr.orphan {
  background: color-mix(in srgb, #b4452c 5%, transparent);
}
.file-cell a {
  color: var(--ink);
