...
<template>
  <section class="series-page section-wrap">
    ...
    <div v-else class="series-grid">
      ...
      >
        <div v-if="series.coverImage" class="series-cover" :style="{ backgroundImage: `url(${series.coverImage})` }" />
...
<style scoped>
.series-page { padding: 48px 0 72px; }
.series-head { margin-bottom: 32px; }
.series-kicker {
  display: flex;
  align-items: center;
  gap: 8px;
...
.series-kicker span { color: #ec4899; font-size: 15px; }
.series-head h1 { margin: 8px 0 6px; font-size: 32px; color: var(--ink); }
.series-sub { color: var(--muted); font-size: 14px; }
.series-error { color: #b4452c; }
...
.series-card:hover { transform: translateY(-3px); border-color: #ec4899; }
.series-cover {
  height: 120px;
...
.series-cover.placeholder span { font-size: 40px; color: #ec4899; opacity: 0.6; }
.series-card-body { display: flex; flex-direction: column; gap: 8px; padding: 16px 18px 14px; }
.series-card-body strong { font-size: 17px; }
.series-card-body p {
  color: var(--muted);
  font-size: 13px;
  line-height: 1.6;
