...
function toEntry(item: PostSummary | Dish | AdminNoteSummary, type: ArchiveContentType): ArchiveEntry {
  if (type === 'ARTICLE') {
    ...
    return {
      ...
      title: p.title,
      summary: p.excerpt,
      publishedAt: normalizeDate(p, type),
      url: `/articles/${p.slug}`,
      category: p.category,
      tags: p.tags,
    };
  ...
  if (type === 'DISH') {
    ...
    return {
      ...
      title: d.name,
      summary: d.summary,
      publishedAt: normalizeDate(d, type),
      url: `/recipes?dish=${d.slug}`,
      category: d.category,
      tags: [],
    };
  }
  ...
  return {
    ...
    title: n.title,
    summary: '',
    publishedAt: normalizeDate(n, type),
    url: `/notes?note=${n.id}`,
    category: n.folder,
    tags: n.tags,
  };
...
const allEntries = computed(() => {
  ...
  if (selectedRelation.value) {
    ...
    filtered = result.filter(
      (e) => e.tags.some((t) => t.toLowerCase() === rel) || e.category.toLowerCase() === rel,
    );
  }
...
async function load() {
  ...
  if (postResult.error && dishResult.error && (noteResult.error || !includeNotes)) {
    ...
    const failed = [
      postResult.error ? '文章' : '',
      dishResult.error ? '菜谱' : '',
      noteResult.error ? '学习笔记' : '',
    ].filter(Boolean);
    partialError.value = failed.length
      ? `部分内容（${failed.join('、')}）加载失败，以下列表可能不完整。`
      : '';
  }
  loading.value = false;
}
...
<template>
  <section class="archive-page section-wrap">
    ...
    <div class="archive-graph-view">
      <KnowledgeGraph :selected-relation="selectedRelation" @select-tag="handleSelectTag" />
    </div>
      ...
      <div class="archive-toolbar">
        <div class="toolbar-left">
          ...
          <span v-if="selectedRelation" class="relation-pill">
            关联: {{ selectedRelation }}
            <button type="button" aria-label="清除关联筛选" @click="updateQuery({ relation: null })">
              ✕
            </button>
          </span>
        </div>
          ...
          <div v-for="monthGroup in yearGroup.months" :key="monthGroup.month" class="archive-month">
            ...
            <div class="archive-list">
              <a
                ...
                class="archive-entry"
              >
                <span class="entry-type" :class="`type-${entry.type.toLowerCase()}`">{{
...
.archive-partial-notice button:hover {
...
.archive-head h1 {
  margin: 16px 0 0;
  font:
    400 clamp(2.5rem, 5vw, 4.2rem)/1.04 Georgia,
    'Songti SC',
    serif;
...
.year-title {
  font:
    520 clamp(1.8rem, 3.5vw, 2.8rem)/1 Georgia,
    'Songti SC',
    serif;
