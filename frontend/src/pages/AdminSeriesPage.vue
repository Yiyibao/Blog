...
const form = reactive<SeriesPayload>({
  name: '',
  slug: '',
  description: '',
  coverImage: null,
...
async function saveEntries() {
  ...
  try {
    applyCurrent(
      await setSeriesEntries(
        current.value.id,
        current.value.version,
        workingEntries.value.map((entry) => ({
          postId: entry.postId,
      ...
      <div v-else class="series-layout">
        <aside class="series-list">
          ...
          >
            ...
            <small>
              <i :class="series.status === 'PUBLISHED' ? 'pub' : 'draft'">{{
        ...
        <div v-if="creating || current" class="series-editor">
          <form class="series-form" @submit.prevent="saveSeries">
            ...
            <div class="form-grid">
              ...
              <label class="wide"
                >简介<textarea v-model="form.description" maxlength="5000" rows="2" />
              </label>
              <label class="wide"
                >封面地址（可空）<input
                  :value="form.coverImage ?? ''"
                  placeholder="https://… 或 /images/…"
                  @input="form.coverImage = ($event.target as HTMLInputElement).value || null"
              /></label>
            </div>
            <footer>
              <button v-if="!creating && current" type="button" class="danger" @click="removeSeries(current)">
                删除合集
              </button>
            ...
            <ul class="entry-rows">
              ...
              >
                ...
                <input
                  ...
                  placeholder="章节标题（可空）"
                  @input="
                    entry.chapterTitle = ($event.target as HTMLInputElement).value || null;
                    entriesDirty = true;
                  "
                />
...
.series-form footer button.primary,
.entry-add button.primary {
  border: none;
  background: var(--accent);
  color: #fff;
}
.series-form footer button.danger {
  color: #b4452c;
