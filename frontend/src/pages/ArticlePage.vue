...
watch(
  [() => content.currentPost, () => content.articleDetailLoading],
  ([post]) => {
    if (post) {
...
<template>
  ...
  <template v-if="content.currentPost">
    <article class="article-page">
      <header class="article-header section-wrap">
        ...
        <div class="post-meta">
          <span>{{ content.currentPost.category }}</span
          ><time v-if="content.currentPost.date">{{ content.currentPost.date }}</time
          ><span v-if="content.currentPost.readTime">{{ content.currentPost.readTime }} MIN READ</span>
        </div>
        ...
        <div class="article-header-actions">
          <div class="tag-row">
            <RouterLink
              v-for="tag in content.currentPost.tags"
              :key="tag"
              class="tag-link"
          ...
          <button
            ...
            type="button"
            @click="
              content.toggleFavorite(content.currentPost.slug);
              ui.showToast(content.favorites.includes(content.currentPost.slug) ? '已收藏' : '已取消收藏');
            "
          >
            {{ content.favorites.includes(content.currentPost.slug) ? '★ 已收藏' : '☆ 收藏' }}
          </button>
        </div>
      </header>
      <div class="article-cover section-wrap" :style="{ '--post-color': content.currentPost.color }">
        <b>{{ content.currentPost.number }}</b
        ><span>HXNF / FIELD NOTE</span><i />
      </div>
      <!-- 4B：合集归属条 -->
      <RouterLink v-if="seriesRef" class="article-series-bar section-wrap" :to="`/series/${seriesRef.slug}`">
        ...
        <span
          >本文属于合集 <strong>{{ seriesRef.name }}</strong
          >（{{ seriesRef.position }}/{{ seriesRef.total }}）</span
        >
      ...
      <div class="article-layout section-wrap">
        <aside class="article-aside">
          ...
          <div class="article-share">
            <p>分享文章</p>
            <button type="button" @click="copyCurrentLink">复制链接</button>
          </div>
        </aside>
      ...
      <nav
        ...
        class="article-neighbors section-wrap"
        aria-label="相邻文章"
      >
        <RouterLink
          v-if="neighbors.previous"
          class="neighbor-link prev"
          :to="`/articles/${neighbors.previous.slug}`"
        >
      ...
      <section v-if="relatedPosts.length" class="related section-wrap">
        <div class="section-heading">
          <p><span>+</span> 继续阅读</p>
        </div>
        <div class="related-grid">
          <RouterLink v-for="post in relatedPosts" :key="post.slug" :to="`/articles/${post.slug}`"
            ><small>{{ post.category }} · {{ post.readTime }} 分钟</small><strong>{{ post.title }}</strong
            ><span>阅读全文 ↗</span></RouterLink
          >
        </div>
      </section>
    </article>
  ...
  <template v-else>
    ...
    <section class="page-hero section-wrap compact-hero">
      <p class="eyebrow">
        <span /> {{ content.contentReady && !content.articleDetailLoading ? 'NOT FOUND' : 'LOADING' }}
      </p>
      <h1>
        {{ content.contentReady && !content.articleDetailLoading ? '这篇文章不存在，' : '正在加载文章，'
        }}<br /><em>{{
          content.contentReady && !content.articleDetailLoading ? '或者已经被归档。' : '请稍候…'
        }}</em>
      </h1>
      <p v-if="content.contentReady && !content.articleDetailLoading">
        链接可能已失效，回到归档继续浏览其他内容。
      </p>
      <RouterLink
        v-if="content.contentReady && !content.articleDetailLoading"
...
.article-series-bar:hover {
  border-color: #ec4899;
}
.article-series-bar .series-icon {
  color: #ec4899;
  font-size: 16px;
}
.article-series-bar strong {
  color: var(--ink);
}
.article-series-bar .series-go {
  margin-left: auto;
  color: var(--muted);
  font-size: 12px;
}
