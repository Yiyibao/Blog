...
<template>
  ...
  <section class="archive section-wrap">
    <p v-if="content.contentError" class="content-unavailable" role="alert">
      文章服务暂时不可用，请确认后端已启动后重试。
    </p>
    <div class="archive-tools">
      <label class="search-field"
        ><span>搜索</span><input v-model="content.query" type="search" placeholder="标题、标签或关键词…"
      /></label>
      <div class="archive-filter-tabs">
        <div class="category-tabs" aria-label="文章分类">
          <button
            ...
            :key="item"
            type="button"
            :class="{ active: content.category === item && !content.showFavoritesOnly }"
            @click="
              content.showFavoritesOnly = false;
        ...
        <button
          class="fav-filter"
          type="button"
          :class="{ active: content.showFavoritesOnly }"
          @click="content.showFavoritesOnly = !content.showFavoritesOnly"
        >
          ★ 收藏 ({{ content.favorites.length }})
        </button>
      </div>
      <label class="sort-field"
        >排序<select v-model="content.sortOrder">
    ...
    <p class="result-count">
      {{ content.archiveTotal.toString().padStart(2, '0') }} RESULTS · PAGE
      {{
        Math.min(content.archivePage + 1, content.archiveTotalPages)
          .toString()
          .padStart(2, '0')
      }}/{{ content.archiveTotalPages.toString().padStart(2, '0') }}
    </p>
    <div class="archive-list">
      ...
      <article
        v-for="(post, index) in content.archivePosts"
        :key="post.slug"
        class="project-card article-project-card"
        :style="{ '--project-color': post.color }"
      >
        ...
        <div class="article-project-copy">
          ...
          <h2>
            <RouterLink :to="`/articles/${post.slug}`">{{ post.title }}</RouterLink>
          </h2>
          <p>{{ post.excerpt }}</p>
          ...
          <div class="article-project-actions">
            ...
            <button
              class="save-button"
              type="button"
              :class="{ saved: content.favorites.includes(post.slug) }"
              :aria-label="content.favorites.includes(post.slug) ? '取消收藏' : '收藏'"
              @click="
                content.toggleFavorite(post.slug);
                ui.showToast(content.favorites.includes(post.slug) ? '已收藏' : '已取消收藏');
              "
            >
              {{ content.favorites.includes(post.slug) ? '★' : '☆' }}
            </button>
          </div>
        </div>
        <RouterLink
          class="project-mark article-project-mark"
          :to="`/articles/${post.slug}`"
          :aria-label="`阅读${post.title}`"
        >
          <i /><span
            >{{ post.category
            }}<template v-if="post.readTime"><br />{{ post.readTime }} MIN READ</template></span
          ><b>READ ↗</b>
        </RouterLink>
      </article>
      <div v-if="!content.archiveLoading && !content.archivePosts.length" class="empty-state">
        <b>没有找到文章</b>
        <p>换一个关键词，或者查看全部分类。</p>
        <button
          type="button"
    ...
    <PaginationNav
      ...
      :total-pages="content.archiveTotalPages"
      aria-label="文章分页"
      @change="content.archivePage = $event"
    />
  </section>
</template>
...
.archive-skeleton-card {
  ...
  background: linear-gradient(
    100deg,
    var(--surface) 40%,
    color-mix(in srgb, var(--ink) 6%, var(--surface)) 50%,
    var(--surface) 60%
  );
