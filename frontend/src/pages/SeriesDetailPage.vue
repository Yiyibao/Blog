...
<style scoped>
.series-detail { padding: 48px 0 72px; }
...
.detail-head { margin: 20px 0 28px; }
.detail-head h1 { margin: 8px 0 8px; font-size: 30px; color: var(--ink); }
.detail-desc { color: var(--muted); font-size: 14px; line-height: 1.7; max-width: 640px; }
.detail-head small { display: block; margin-top: 8px; color: var(--muted); font-size: 12px; }
.entry-list {
  list-style: none;
  margin: 0;
  padding: 0;
...
.entry-link:hover { border-color: #ec4899; }
.entry-pos {
...
.entry-main { display: flex; flex-direction: column; gap: 2px; flex: 1; }
.entry-main small { color: var(--muted); font-size: 12px; }
.entry-main strong { font-size: 15px; }
.entry-link time { color: var(--muted); font-size: 12px; }
</style>
