...
import {
  ...
  fetchAdminTracks, createAdminTrack, updateAdminTrack, deleteAdminTrack,
  fetchAdminQuotes, createAdminQuote, updateAdminQuote, deleteAdminQuote,
  type AdminMusicTrack, type AdminQuote, type MusicTrackPayload, type QuotePayload,
} from '../api/admin'
...
const trackForm = reactive<MusicTrackPayload>({
  trackId: '', title: '', artist: '', duration: 0, audioUrl: '', coverUrl: '', sortOrder: 0,
})
...
function resetTrackForm() {
  ...
  Object.assign(trackForm, {
    trackId: '', title: '', artist: '', duration: 0, audioUrl: '', coverUrl: '',
...
function editTrack(track: AdminMusicTrack) {
  ...
  Object.assign(trackForm, {
    trackId: track.trackId, title: track.title, artist: track.artist, duration: track.duration,
    audioUrl: track.audioUrl, coverUrl: track.coverUrl, sortOrder: track.sortOrder,
  })
}
...
function editQuote(quote: AdminQuote) {
  ...
  Object.assign(quoteForm, {
    content: quote.content, author: quote.author, category: quote.category, displayOrder: quote.displayOrder,
  })
}
      ...
      <template v-else>
        <section class="library-block">
          ...
          <table class="library-table">
            <thead><tr><th>#</th><th>标题</th><th>艺人</th><th>音频地址</th><th>排序</th><th /></tr></thead>
            <tbody>
              <tr v-for="track in tracks" :key="track.id" :class="{ placeholder: isPlaceholderUrl(track.audioUrl) }">
                <td>{{ track.trackId }}</td>
                <td>{{ track.title }}</td>
                <td>{{ track.artist }}</td>
                <td class="url-cell">
          ...
          <form class="library-form" @submit.prevent="saveTrack">
            ...
            <div class="form-grid">
              <label>标识<input v-model="trackForm.trackId" required pattern="[a-z0-9-]+" placeholder="soft-piano"></label>
              ...
              <label>时长（秒）<input v-model.number="trackForm.duration" type="number" min="0" max="36000"></label>
              <label class="wide">音频地址<input v-model="trackForm.audioUrl" required placeholder="https://… 或 /audio/…"></label>
              <label class="wide">封面地址（可空）<input v-model="trackForm.coverUrl" placeholder="https://… 或 /images/…"></label>
              <label>排序<input v-model.number="trackForm.sortOrder" type="number" min="0" max="9999"></label>
            </div>
            <footer>
              <button v-if="editingTrackId" type="button" @click="resetTrackForm">取消编辑</button>
              <button class="primary" type="submit" :disabled="saving">{{ saving ? '保存中…' : '保存曲目' }}</button>
            </footer>
          </form>
        </section>
        ...
        <section class="library-block">
          ...
          <table class="library-table">
            <thead><tr><th>语录</th><th>作者</th><th>分类</th><th>排序</th><th /></tr></thead>
            <tbody>
          ...
          <form class="library-form" @submit.prevent="saveQuote">
            ...
            <div class="form-grid">
              <label class="wide">语录内容<textarea v-model="quoteForm.content" required maxlength="1000" rows="2" /></label>
              ...
              <label>排序<input v-model.number="quoteForm.displayOrder" type="number" min="0" max="9999"></label>
            </div>
            <footer>
              <button v-if="editingQuoteId" type="button" @click="resetQuoteForm">取消编辑</button>
              <button class="primary" type="submit" :disabled="saving">{{ saving ? '保存中…' : '保存语录' }}</button>
            </footer>
          </form>
        </section>
      </template>
...
.block-head h2 { margin: 0; font-size: 17px; color: var(--ink); }
...
.library-table th { color: var(--muted); font-weight: 600; font-size: 12px; }
.url-cell { max-width: 280px; word-break: break-all; }
.url-cell b { color: #b4452c; font-size: 11px; margin-left: 6px; }
tr.placeholder { background: color-mix(in srgb, #c47b2c 6%, transparent); }
.quote-cell { max-width: 380px; }
...
.row-actions button.danger { color: #b4452c; border-color: color-mix(in srgb, #b4452c 40%, var(--line)); }
.library-form {
  padding: 14px;
  border: 1px dashed var(--line-strong);
  border-radius: 12px;
}
.library-form strong { display: block; margin-bottom: 10px; font-size: 13px; color: var(--ink); }
.form-grid {
  display: grid;
