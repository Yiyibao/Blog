...
<template>
  <section class="hero-stage">
    <div class="hero" :style="heroStyle">
      ...
      <div class="hero-copy">
        ...
        <h1 class="hero-enter" style="--enter-delay: 80ms">
          把日常散落的微光<span class="hero-punctuation">，</span><br /><em
            >写成可以重读的记忆<span class="hero-punctuation">。</span></em
          >
        </h1>
        <p class="hero-intro hero-enter" style="--enter-delay: 160ms">
      ...
      <a class="hero-scroll" href="#home-content" aria-label="向下浏览"
  ...
  <div id="home-content" class="home-content">
    <p
      v-if="content.contentError && !content.posts.length"
      class="content-unavailable section-wrap"
      role="alert"
    >
      内容服务暂时不可用，请稍后刷新重试。
    </p>
    <p v-else-if="content.contentReady && !content.posts.length" class="content-unavailable section-wrap">
      这里还没有发布内容，敬请期待。
    </p>

    <!-- NF-4：生产环境无内置种子——首个响应到达前用骨架占位，避免布局跳动 -->
    <section
      v-if="!content.contentReady && !content.posts.length"
      class="featured section-wrap"
      aria-hidden="true"
    >
    ...
    <section v-if="content.featuredPost" id="featured-story" class="featured section-wrap band-featured">
      <div class="section-heading">
        <p><span>01</span> 本期精选</p>
        <RouterLink to="/articles">浏览全部 ↗</RouterLink>
      </div>
      <article class="featured-card">
        <div class="featured-visual" :style="{ '--post-color': content.featuredPost.color }">
          <span>{{ content.featuredPost.number }}</span
          ><i /><small>FEATURED<br />STORY</small>
        </div>
        <div class="featured-content">
          <div class="post-meta">
            <span>{{ content.featuredPost.category }}</span
            ><time>{{ content.featuredPost.date }}</time
            ><span>{{ content.featuredPost.readTime }} MIN READ</span>
          </div>
          <h2>
            <RouterLink :to="`/articles/${content.featuredPost.slug}`">{{
              content.featuredPost.title
            }}</RouterLink>
          </h2>
          <p>{{ content.featuredPost.excerpt }}</p>
          <div class="tag-row">
            <span v-for="tag in content.featuredPost.tags" :key="tag"># {{ tag }}</span>
          </div>
          <RouterLink class="text-link" :to="`/articles/${content.featuredPost.slug}`"
            >阅读全文 <b>↗</b></RouterLink
          >
        </div>
      </article>
    ...
    <section class="latest section-wrap band-latest">
      <div class="section-heading">
        <p><span>02</span> 最近更新</p>
        <span class="heading-note">NOTES FROM THE PROCESS</span>
      </div>
      <div class="post-grid">
        <article v-for="post in content.posts.slice(1, 4)" :key="post.slug" class="post-card">
          <div class="mini-cover" :style="{ '--post-color': post.color }">
            <b>{{ post.number }}</b
            ><i />
          </div>
          <div class="post-meta">
