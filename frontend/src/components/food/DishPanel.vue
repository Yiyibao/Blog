...
function scaledAmount(item: string): string {
  ...
  return item.replace(
    /(\d+(?:\.\d+)?)\s*(克|毫升|ml|g|kg|个|根|片|瓣|勺|汤匙|茶匙|小匙|大匙|碗|杯|只|条|块|包)/g,
        ...
        >
          <header class="dish-panel-media">
            ...
            <button
              ...
              @click="onHeart"
            >
              <i aria-hidden="true">♥</i><b>{{ dish.favoriteCount }}</b>
            </button>
            <button
              v-if="canCheckIn"
            ...
            <div>
              <small>{{ dish.category }} · ★ {{ dish.rating.toFixed(1) }}</small>
              <h2 :id="`dish-title-${dish.id}`">{{ dish.name }}</h2>
              <p>{{ dish.summary }}</p>
            </div>
          </header>
          <div class="dish-panel-body">
            <dl>
              <div>
                <dt>准备时间</dt>
                <dd>{{ dish.prepMinutes }} 分钟</dd>
              </div>
              <div>
                <dt>难度</dt>
                <dd>{{ dish.difficulty }}</dd>
              </div>
              <div>
                <dt>食材</dt>
                <dd>{{ dish.ingredients.length }} 项</dd>
              </div>
            </dl>
            <section>
              <p>01 / INGREDIENTS</p>
              ...
              <ul>
                <li v-for="(item, index) in dish.ingredients" :key="index">
                  <button
                    type="button"
...
.dish-panel {
  ...
  box-shadow: -40px 0 100px rgba(0, 0, 0, 0.46);
}
.dish-panel-media {
  position: relative;
  min-height: 410px;
...
.dish-panel-media > div {
  position: absolute;
  right: 36px;
  bottom: 34px;
  left: 36px;
}
.dish-panel-media small {
...
.dish-panel-body dl div {
  padding: 0 16px;
  border-left: 1px solid var(--cinema-line);
}
.dish-panel-body dl div:first-child {
  padding-left: 0;
  border-left: 0;
}
.dish-panel-body dt {
...
.dish-panel-body section {
  margin-top: 50px;
}
.dish-panel-body section > p {
...
.dish-heart-btn:hover {
  transform: scale(1.06);
  border-color: rgba(255, 143, 168, 0.6);
}
.dish-heart-btn.bumping i {
...
.servings-bar button:hover:not(:disabled) {
  border-color: var(--accent);
  color: var(--accent);
}
.servings-bar button:disabled {
  opacity: 0.3;
  cursor: default;
}
.servings-bar span {
...
.dish-panel-body ol span {
  color: var(--cinema-faint);
  font-size: 0.72rem;
}
.dish-panel-body ol p {
  margin: 0;
  color: var(--cinema-text);
  line-height: 1.75;
}
.dish-panel-media > button:focus-visible,
.servings-bar button:focus-visible,
.dish-heart-btn:focus-visible,
...
.dish-panel-leave-active {
  transition:
    background 0.38s,
    backdrop-filter 0.38s;
}
.dish-panel-enter-active .dish-panel,
.dish-panel-leave-active .dish-panel {
  transition:
...
.dish-panel-enter-from .dish-panel,
.dish-panel-leave-to .dish-panel {
  opacity: 0.55;
