...
watch(() => props.dish, async (dish, previous) => {
  if (dish) {
    if (dish.slug !== previous?.slug) servings.value = 2
...
.dish-panel-body ul li { padding: 14px 0; color: var(--cinema-text); border-top: 1px solid var(--cinema-line); }
...
.dish-panel-media > button:focus-visible, .servings-bar button:focus-visible, .dish-panel-body footer a:focus-visible { outline: 2px solid #0071e3; outline-offset: 3px; }
...
@media (max-width: 640px) { .dish-panel-media { min-height: 360px; } .dish-panel-media > div { right: 24px; bottom: 26px; left: 24px; } .dish-panel-body ul { grid-template-columns: 1fr; } }
@media (prefers-reduced-motion: reduce) { .dish-panel-enter-active, .dish-panel-leave-active, .dish-panel-enter-active .dish-panel, .dish-panel-leave-active .dish-panel, .dish-panel { transition-duration: .01ms !important; } }
</style>
