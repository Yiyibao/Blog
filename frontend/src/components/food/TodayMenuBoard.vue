...
<template>
  <Teleport to="body">
    <div class="board-backdrop" @click.self="emit('close')">
      ...
      >
        <button class="board-close tap-44" type="button" aria-label="关闭菜单编辑" @click="emit('close')">
          ×
        </button>
        ...
        <ul v-if="store.menu?.items.length" class="board-items">
          <li v-for="item in store.menu.items" :key="item.id" :class="{ pending: item.id < 0 }">
            <i class="board-slot" aria-hidden="true">{{
              SLOTS.find((s) => s.value === item.mealSlot)?.label ?? item.mealSlot
            }}</i>
            <div class="board-copy">
              <strong>{{ item.title }}</strong>
              <small
                >{{ item.authorName }} 点的<template v-if="item.note"> · {{ item.note }}</template></small
              >
            </div>
            <button
...
.menu-board {
  ...
  animation: board-in 0.5s var(--ease-out) both;
}
.board-close {
  position: absolute;
  top: 14px;
...
.menu-board h2 {
  margin: 10px 0 18px;
  font:
    400 clamp(1.7rem, 4vw, 2.2rem)/1.1 Georgia,
    'Songti SC',
    serif;
...
.board-items li:first-child {
  border-top: 0;
}
.board-items li.pending {
  opacity: 0.55;
}
.board-slot {
  flex: 0 0 auto;
...
.board-remove:hover:not(:disabled) {
  color: #b84f48;
  border-color: #b84f48;
}
.board-remove:disabled {
  opacity: 0.4;
  cursor: default;
}
.board-empty {
...
.board-confirm:hover:not(:disabled) {
  color: #fff;
  background: var(--accent);
}
