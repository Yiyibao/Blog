...
function draw() {
  ...
  spinTimer = window.setTimeout(() => { phase.value = 'settled' }, SPIN_MS)
}
...
<template>
  <Teleport to="body">
    <div class="roulette-backdrop" @click.self="emit('close')">
      <div ref="dialogRoot" class="roulette-dialog" role="dialog" aria-modal="true" aria-labelledby="roulette-title">
        <button class="roulette-close tap-44" type="button" aria-label="关闭抽卡" @click="emit('close')">×</button>
        ...
        <template v-else>
          ...
          <p class="roulette-status" role="status">
            {{ phase === 'settled' && result ? `抽到了：${result.name}` : phase === 'spinning' ? '命运的滚轮转起来了…' : '' }}
          </p>
        </template>
        ...
        <div class="roulette-actions">
          <button
            ...
            :disabled="!canSpin"
            @click="draw"
          >{{ phase === 'spinning' ? '转着呢…' : '开抽！' }}</button>
          <template v-else>
            <button class="roulette-open tap-44" type="button" @click="result && emit('open', result)">就吃这道 ↗</button>
...
<style scoped>
.roulette-backdrop { position: fixed; z-index: 2300; inset: 0; display: grid; place-items: center; padding: 20px; background: var(--cinema-backdrop); backdrop-filter: blur(8px); }
.roulette-dialog { position: relative; width: min(460px, 100%); padding: clamp(26px, 5vw, 40px); color: var(--ink); background: var(--surface-solid); border: 1px solid color-mix(in srgb, var(--accent) 22%, var(--line)); border-radius: 28px 28px 9px 28px; box-shadow: var(--shadow-lg); animation: roulette-in .5s var(--ease-out) both; }
.roulette-close { position: absolute; top: 14px; right: 14px; display: grid; place-items: center; color: var(--muted); font-size: 1.2rem; background: transparent; border: 1px solid var(--line); border-radius: 50%; cursor: pointer; transition: color .2s, border-color .2s; }
.roulette-close:hover { color: var(--ink); border-color: var(--line-strong); }
.roulette-kicker { margin: 0; color: var(--accent); font: 650 .64rem/1 ui-monospace, "SF Mono", Consolas, monospace; letter-spacing: .17em; }
.roulette-dialog h2 { margin: 10px 0 22px; font: 400 clamp(1.9rem, 4vw, 2.6rem)/1.05 Georgia, "Songti SC", serif; letter-spacing: -.04em; }
.roulette-empty { margin: 18px 0 6px; color: var(--muted); line-height: 1.7; }
.roulette-reel { position: relative; height: 168px; margin-bottom: 20px; overflow: hidden; border: 1px solid var(--line); border-radius: 16px 16px 5px 16px; background: color-mix(in srgb, var(--accent-soft) 40%, transparent); }
.roulette-reel ul { margin: 0; padding: 0; list-style: none; }
.roulette-reel li { display: grid; place-items: center; height: 56px; font: 500 1.15rem/1 Georgia, "Songti SC", serif; color: var(--muted); }
.roulette-reel ul.spinning { animation: reel-spin .38s linear infinite; }
.reel-window { position: absolute; top: 56px; right: 10px; left: 10px; height: 56px; border: 1px dashed color-mix(in srgb, var(--accent) 55%, transparent); border-radius: 10px; pointer-events: none; }
.roulette-result { display: grid; grid-template-columns: 108px 1fr; gap: 16px; align-items: center; margin-bottom: 16px; padding: 12px; border: 1px solid color-mix(in srgb, var(--accent) 30%, var(--line)); border-radius: 18px 18px 6px 18px; background: color-mix(in srgb, var(--accent-soft) 46%, transparent); animation: roulette-settle .45s cubic-bezier(.175, .885, .32, 1.275) both; }
.roulette-result img { width: 108px; height: 96px; object-fit: cover; border-radius: 12px 12px 4px 12px; }
.roulette-result-copy { display: flex; flex-direction: column; gap: 6px; min-width: 0; }
.roulette-result-copy small { color: var(--faint); font-size: .68rem; letter-spacing: .05em; }
.roulette-result-copy strong { font: 520 1.4rem/1.2 Georgia, "Songti SC", serif; letter-spacing: -.03em; }
...
@media (prefers-reduced-motion: reduce) { .roulette-dialog, .roulette-result { animation: none; } .roulette-reel ul.spinning { animation: none; } .roulette-actions button { transition-duration: .01ms; } }
</style>
