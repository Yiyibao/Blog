...
.menu-card {
  ...
  background: linear-gradient(
    160deg,
    color-mix(in srgb, var(--surface-solid) 90%, var(--accent-soft)),
...
.menu-seal {
  ...
  background: radial-gradient(
    ...
    var(--accent) 62%,
    color-mix(in srgb, var(--accent) 72%, #7a2f42)
  );
  border-radius: 50%;
  font:
    500 1.05rem/1 Georgia,
    'Songti SC',
    serif;
  box-shadow:
    0 4px 12px color-mix(in srgb, var(--accent) 45%, transparent),
...
.menu-lines li:first-child {
  border-top: 0;
}
.menu-lines li.pending {
  opacity: 0.55;
}
.menu-lines li.arrived {
  animation: menu-chip-arrive 0.8s var(--ease-out);
}
.menu-slot {
  flex: 0 0 auto;
  display: grid;
...
.menu-card.confirmed .menu-status {
  color: var(--accent);
  font-weight: 600;
}
.menu-open {
...
.menu-plates i:nth-child(2) {
  animation: menu-plate-breathe 6s ease-in-out infinite;
}
.menu-loading {
  display: flex;
  flex-direction: column;
