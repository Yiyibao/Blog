...
import { classifyError, deleteMealLog, fetchMealLogs, type MealLog, type MealSlot } from '../../api/kitchen';
  ...
  <section v-if="logs.length || loading" class="food-timeline" aria-labelledby="food-timeline-title">
    ...
    <ol class="timeline-reel">
      <li
        ...
        :key="group.date"
        class="timeline-day"
        :class="{ today: dateLabel(group.date) === '今天' }"
      >
        <h3>
          <time :datetime="group.date">{{ dateLabel(group.date) }}</time
          ><i v-if="dateLabel(group.date) === '今天'" class="today-pulse" aria-hidden="true" />
        </h3>
        <ul>
...
.timeline-head h2 {
  ...
  color: var(--ink);
  font:
    400 clamp(2rem, 4vw, 3.2rem)/1.05 Georgia,
    'Songti SC',
    serif;
...
.timeline-reel::before {
  ...
  background: repeating-linear-gradient(
    180deg,
    color-mix(in srgb, var(--accent) 34%, var(--line)) 0 6px,
...
.timeline-entry:hover .entry-remove,
...
.timeline-more:hover:not(:disabled) {
  color: var(--accent);
...
.food-timeline-empty p:last-child {
  margin: 0;
  color: var(--muted);
  font-size: 0.9rem;
  line-height: 1.7;
