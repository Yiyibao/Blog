...
<template>
  <div class="graph-search glass-panel">
    <div class="search-input-wrapper">
      ...
      />
      <button v-if="query" type="button" class="clear-btn" aria-label="清除搜索" @click="query = ''; isOpen = false">
        ✕
      </button>
    </div>
    ...
    <ul v-if="isOpen && results.length > 0" class="search-results-dropdown glass-card">
      <li
        ...
        :key="node.id"
        class="result-item"
        @mousedown.prevent="handleSelect(node)"
      >
        <span class="result-badge" :style="{ backgroundColor: node.color }">
          {{ node.type === 'POST' ? '文章' : node.type === 'NOTE' ? '笔记' : node.type === 'DISH' ? '菜谱' : node.type === 'SERIES' ? '合集' : '标签' }}
        </span>
