...
async function flushCurrent() {
  if (!selectedId.value || (saveState.value === 'saved' && !savePromise && savedRevision === editRevision)) return true
...
async function loadAttachments(noteId: number) {
  ...
  try {
    ...
    const previews = await Promise.all(loaded.map(async attachment => {
...
async function removeAttachment(attachment: NoteAttachment) {
  if (!selectedId.value || pendingUploads > 0 || !window.confirm(`确认删除图片“${attachment.fileName}”？笔记中已插入的引用将失效。`)) return
  const noteId = selectedId.value
  attachmentLoadRevision += 1
  try {
    ...
    if (selectedId.value === noteId) {
      ...
      if (previewUrl) {
        applying = true
        form.markdownContent = replaceCanonicalUrls(form.markdownContent, [attachment], { [attachment.id]: previewUrl })
        URL.revokeObjectURL(previewUrl)
      }
...
async function load() {
  ...
  if (!await flushCurrent()) return
  try {
    const result = await fetchNotes(notePage.value, notePageSize, statusFilter.value === 'ALL' ? '' : statusFilter.value)
...
async function saveNow(): Promise<AdminNote | null> {
  ...
  try {
    ...
    savePromise = null
    if (selectedId.value === noteId && savedRevision < editRevision && saveState.value !== 'error' && !publicationChanging) {
      void saveNow()
    }
  }
}
...
async function changePublication(nextStatus: NoteStatus) {
  ...
  try {
    ...
    const changed = nextStatus === 'PUBLISHED'
    ...
    saveState.value = 'saved'
    publicationNotice.value = nextStatus === 'PUBLISHED'
      ? '笔记已发布，公开页面现在可以阅读。'
      : nextStatus === 'ARCHIVED' ? '笔记已归档，仅后台可见。' : '笔记已恢复为草稿，仅后台可见。'
    noticeTimer = window.setTimeout(() => { publicationNotice.value = '' }, 3200)
  } catch (cause) {
    handleError(cause, nextStatus === 'PUBLISHED' ? '发布失败，请稍后重试。' : '状态变更失败，请稍后重试。')
  } finally {
    publicationChanging = false
    publishing.value = false
    if (selectedId.value === noteId && savedRevision < editRevision && saveState.value !== 'error') void saveNow()
  }
}
...
async function removeCurrent() {
  ...
  try {
    ...
    deleting = false
    if (selectedId.value === removedId && savedRevision < editRevision && saveState.value !== 'error') void saveNow()
  }
}
...
watch(statusFilter, () => { notePage.value = 0; void load() })
function warnBeforeUnload(event: BeforeUnloadEvent) {
  if (pendingUploads === 0 && saveState.value === 'saved' && !savePromise && savedRevision === editRevision) return
    ...
    <main class="note-desk">
      ...
      <template v-else>
        <header class="note-topbar">
          <div class="note-meta-fields"><input v-model="form.folder" aria-label="目录" placeholder="目录"><span>/</span><span class="note-status-label">{{ statusLabel }}</span></div>
          <div class="note-save-state" :class="saveState"><i />{{ saveLabel }}</div>
          <div class="note-top-actions"><button title="专注模式 Ctrl+Shift+F" @click="focusMode = !focusMode">{{ focusMode ? '退出专注' : '专注' }}</button><button @click="selected && exportNote(selected)">导出 .md</button><button class="danger" @click="removeCurrent">删除</button><button class="save-note" @click="saveNow">保存</button><button v-if="form.status === 'DRAFT'" class="publish-note" :disabled="publishing || saveState === 'saving'" @click="changePublication('PUBLISHED')">{{ publishing ? '正在发布…' : '发布笔记' }}</button><button v-if="form.status === 'DRAFT'" :disabled="publishing || saveState === 'saving'" @click="changePublication('ARCHIVED')">归档</button><button v-else-if="form.status === 'PUBLISHED'" class="publish-note unpublish" :disabled="publishing || saveState === 'saving'" @click="changePublication('DRAFT')">{{ publishing ? '正在撤回…' : '撤回公开' }}</button><button v-else class="publish-note unpublish" :disabled="publishing || saveState === 'saving'" @click="changePublication('DRAFT')">{{ publishing ? '正在恢复…' : '恢复草稿' }}</button></div>
        </header>
