...
function editMath(kind: 'inline' | 'block', latex = '') {
  const value = window.prompt(
    kind === 'inline' ? '输入行内 LaTeX 公式' : '输入块级 LaTeX 公式',
    latex || 'E = mc^2',
  );
...
<template>
  <div class="typora-editor">
    <div class="typora-toolbar" role="toolbar" aria-label="Markdown 格式工具栏">
      ...
      <div class="tool-group">
        <button title="无序列表" @click="editor?.chain().focus().toggleBulletList().run()">
          <span class="tool-icon">•</span><span>列表</span>
        </button>
        <button title="有序列表" @click="editor?.chain().focus().toggleOrderedList().run()">
          <span class="tool-icon">1.</span><span>列表</span>
        </button>
        <button title="任务列表" @click="editor?.chain().focus().toggleTaskList().run()">
          <span class="tool-icon">☑</span><span>任务</span>
        </button>
        <button title="引用" @click="editor?.chain().focus().toggleBlockquote().run()">
          <span class="tool-icon">❝</span><span>引用</span>
        </button>
        <button
        ...
        >
          <span class="tool-icon">{ }</span><span>代码</span>
        </button>
        <select
      ...
      <div class="tool-group">
        ...
        >
          <span class="tool-icon">▦</span><span>表格</span>
        </button>
        <button title="上传图片，也可直接粘贴或拖入" @click="imageInput?.click()">
          <span class="tool-icon">▧</span><span>图片</span>
        </button>
        <button title="插入数学公式" @click="editMath('block')">
          <span class="tool-icon">∑</span><span>公式</span>
        </button>
        <button title="分隔线" @click="editor?.chain().focus().setHorizontalRule().run()">
          <span class="tool-icon">—</span>
        </button>
      </div>
      <div class="tool-group tool-history">
        <button :disabled="!can?.can().undo()" title="撤销" @click="editor?.chain().focus().undo().run()">
          ↶
        </button>
        <button :disabled="!can?.can().redo()" title="重做" @click="editor?.chain().focus().redo().run()">
          ↷
        </button>
      </div>
      <button
        ...
        title="Markdown 源码模式 Ctrl+Shift+M"
        @click="toggleSource"
      >
        <span class="tool-icon">&lt;/&gt;</span>
    ...
    <div v-if="slashOpen && !sourceMode" class="slash-menu">
      ...
      <button @click="runSlash('text')">
        <b>¶</b><span>正文<small>普通文本段落</small></span>
      </button>
      <button @click="runSlash('h1')">
        <b>H1</b><span>一级标题<small>章节主标题</small></span>
      </button>
      <button @click="runSlash('h2')">
        <b>H2</b><span>二级标题<small>章节小标题</small></span>
      </button>
      <button @click="runSlash('task')">
        <b>☑</b><span>任务列表<small>可勾选的待办事项</small></span>
      </button>
      <button @click="runSlash('quote')">
        <b>❝</b><span>引用<small>突出一段引文</small></span>
      </button>
      <button @click="runSlash('code')">
        <b>{ }</b><span>代码块<small>多行等宽代码</small></span>
      </button>
      <button @click="runSlash('table')">
        <b>▦</b><span>表格<small>3 × 3 数据表</small></span>
      </button>
      <button @click="runSlash('image')">
        <b>▧</b><span>图片<small>上传本地图片</small></span>
      </button>
      <button @click="runSlash('math')">
        <b>∑</b><span>数学公式<small>KaTeX / LaTeX</small></span>
      </button>
    </div>
    <textarea
      v-if="sourceMode"
