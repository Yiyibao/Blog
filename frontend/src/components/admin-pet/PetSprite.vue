 ...
 *   sourceCellWidth/sourceCellHeight 计算；对外 size 由宿主按桌面/移动场景传入。
...
const hdSource = computed(() =>
  props.state === 'look' ? HD_SOURCE(lookHdRow(props.lookDirection)) : PET_HD_ANIMATIONS[props.state].source,
);
...
const transformRow = computed(() => (fallbackLegacy.value ? fallbackMeta.value.row : 0));
