...
export type KitchenErrorKind = 'auth' | 'forbidden' | 'not-found' | 'conflict' | 'rate-limited' | 'validation' | 'network' | 'server'
...
export function classifyError(cause: unknown): KitchenError {
  ...
  if (axios.isAxiosError(cause)) {
    ...
    if (status === 429) {
      ...
      return classified('rate-limited', serverMessage || '操作太频繁啦，稍后再试',
...
export function fetchMenuHistory(page = 0, size = 20, from?: string, to?: string) {
  return unwrap<MenuHistoryPage>(api.get('/kitchen/menus/history', {
