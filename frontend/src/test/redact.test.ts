...
import {
  redact, redactAndTruncate, redactErrorChain, safeJsonSummary,
...
describe('统一日志脱敏（先脱敏、再截断）', () => {
  ...
  it('异常 cause chain 脱敏（含 code/errno/syscall/port）', () => {
    const cause = Object.assign(new Error('connect ECONNREFUSED 127.0.0.1:9400'), {
      code: 'ECONNREFUSED', errno: -4078, syscall: 'connect', address: '127.0.0.1', port: 9400,
    })
  ...
  it('异常消息中的凭据被脱敏', () => {
    const error = new Error(`auth failed with Authorization: Bearer SUPER_SECRET_BEARER_ABC token=SECRET_TOKEN_789`)
