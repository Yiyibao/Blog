...
vi.mock('../api/admin', async (importOriginal) => {
  ...
  return {
    ...
    fetchAdminCategories: vi
      .fn()
      .mockResolvedValue([
        { id: 1, name: '工程实践', slug: '工程实践', description: '', postCount: 1, publishedPostCount: 1 },
      ]),
    fetchAdminDishCategories: vi
      .fn()
      .mockResolvedValue([
        { id: 1, name: '十分钟菜', slug: '十分钟菜', description: '', dishCount: 2, publishedDishCount: 2 },
      ]),
...
describe('AdminSidebar Navigation & Highlighting', () => {
  ...
  it('highlights overview on /admin path with no section query', async () => {
    ...
    expect(
      wrapper
        .findAll('nav a')
        .find((link) => link.text().includes('学习笔记'))
        ?.classes(),
  ...
  it('allows a new post slug to be left blank for server-side generation', async () => {
    ...
    const slugLabel = wrapper
      .findAll('.editor-card label')
  ...
  it('opens category management from the posts module', async () => {
    ...
    const manageButton = wrapper
      .findAll('.content-head-actions button')
