...
describe('AdminAiImagesPage 会话侧边栏', () => {
  ...
  it('首次生成自动新建会话并继续在原会话追加', async () => {
    mockGenerateAiImages
      ...
      .mockResolvedValueOnce({
        sessionId: SESSION_ID,
        sessionTitle: '第一句话',
        images: [imageOf('d7c68fa8-9a6b-4b7a-bf4e-000000000005', 'd7c68fa8-9a6b-4b7a-bf4e-000000000006', '第二句话')],
      })
    const wrapper = await mountPage()
    ...
    await flushPromises()
    expect(mockGenerateAiImages).toHaveBeenLastCalledWith(expect.objectContaining({ sessionId: null, prompt: '第一句话' }))

    await wrapper.find('textarea').setValue('第二句话')
    await wrapper.find('button.send-btn').trigger('click')
    await flushPromises()
    expect(mockGenerateAiImages).toHaveBeenLastCalledWith(expect.objectContaining({ sessionId: SESSION_ID, prompt: '第二句话' }))
  ...
  it('确认后删除会话并从列表移除', async () => {
    mockFetchAiImageSessions
      .mockResolvedValueOnce([sessionOf(1, '要被删的会话')])
      .mockResolvedValue([])
