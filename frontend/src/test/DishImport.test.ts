...
const mocks = vi.hoisted(() => ({
  ...
  commitDishImport: vi.fn<(token: string, payload: { category: string; correctedSlug?: string }) => Promise<AdminDish>>(),
...
vi.mock('../api/admin', async (importOriginal) => {
  ...
  return {
    ...
    fetchAdminDishCategories: vi.fn<() => Promise<AdminDishCategory[]>>().mockResolvedValue([
      { id: 1, name: '十分钟菜', slug: 'shifenzhongcai', description: '', dishCount: 5, publishedDishCount: 3 },
    ...
    fetchAdminStats: vi.fn().mockResolvedValue({
      posts: 0, dishes: 0, notes: 0, publishedPosts: 0, draftPosts: 0,
      attachmentCount: 0, attachmentBytes: 0, aiUsage: { requests: 0, tokens: 0 }, viewTrend: [], topPosts: [],
    }),
    logout: vi.fn(),
  }
})
...
function findSlugInput(wrapper: ReturnType<typeof mount>) {
  const labels = wrapper.findAll('.import-modal label')
  const slugLabel = labels.find(l => l.text().includes('Slug'))
  return slugLabel?.find('input')
}
...
describe('commitDishImport', () => {
  it('sends a POST with token and payload and returns AdminDish', async () => {
    const payload = { category: '十分钟菜', correctedSlug: 'mapo-tofu' }
  ...
  it('accepts commit without correctedSlug', async () => {
    mocks.commitDishImport.mockResolvedValue(MOCK_ADMIN_DISH)
...
describe('AdminDashboard import flow', () => {
  ...
  it('pre-fills category from categoryMatch and slug from recipe slug', async () => {
    mocks.previewDishImport.mockResolvedValue(MOCK_PREVIEW)
    ...
    const slugInput = findSlugInput(wrapper)
    expect(slugInput?.element).toBeTruthy()
    expect((slugInput?.element as HTMLInputElement).value).toBe('mapo-tofu')
  ...
  it('allows editing the slug before commit', async () => {
    mocks.previewDishImport.mockResolvedValue(MOCK_PREVIEW)
  ...
  it('calls commitDishImport with token, category, and correctedSlug on commit', async () => {
    mocks.previewDishImport.mockResolvedValue(MOCK_PREVIEW)
    mocks.commitDishImport.mockResolvedValue(MOCK_ADMIN_DISH)
    ...
    expect(mocks.commitDishImport).toHaveBeenCalledWith('preview-token-abc', {
      category: '川菜',
      correctedSlug: 'custom-slug',
    })
  })

  it('calls commitDishImport without correctedSlug when slug is empty', async () => {
    mocks.previewDishImport.mockResolvedValue(MOCK_PREVIEW_NO_SLUG)
    mocks.commitDishImport.mockResolvedValue(MOCK_ADMIN_DISH)
