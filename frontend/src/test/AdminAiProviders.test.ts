...
describe('AdminAiProviders Component', () => {
  ...
  it('edits without re-entering the key: empty password field is omitted from payload', async () => {
    const wrapper = await mountComponent();
    await wrapper
      .findAll('.provider-table article')[0]
  ...
  it('sets a provider as default; disabled providers cannot be selected', async () => {
    ...
    const disabledRowBtn = rows[1]
      .findAll('.provider-actions button')
      .find((btn) => btn.text() === '设为默认')!;
  ...
  it('runs a connectivity test and lists the returned models', async () => {
    ...
    expect(result.findAll('.provider-model-list li').map((item) => item.text())).toEqual([
      'deepseek-chat',
      'deepseek-reasoner',
    ]);
  });
  ...
  it('invalidates a stale connectivity result after the provider is edited and saved', async () => {
    ...
    const wrapper = await mountComponent();
    const rowButtons = () =>
      wrapper.findAll('.provider-table article')[0].findAll('.provider-actions button');
  ...
  it('deletes only after confirmation', async () => {
    ...
    const deleteBtn = () =>
      wrapper
        .findAll('.provider-table article')[2]
        .findAll('.provider-actions button')
  ...
  it('shows env-only credential label for OPENCODE_SERVER', async () => {
    const ocsProvider = {
      ...
      providerType: 'OPENCODE_SERVER' as const,
      hasKey: false,
      keyTail: null,
    };
  ...
  it('preserves providerType when editing an OPENCODE_SERVER provider', async () => {
    ...
    await wrapper
      .findAll('.provider-table article')[1]
      .findAll('.provider-actions button')
      .find((btn) => btn.text() === '编辑')!
      .trigger('click');
  ...
  it('供应商页与全屏聊天页共享同一份模型选择并实时同步', async () => {
    ...
    const pageModel = () =>
      (providersPage.find('[data-testid="page-model-select"]').element as HTMLSelectElement).value;
    const chatModel = () =>
      (chat.find('[data-testid="chat-model-select"]').element as HTMLSelectElement).value;
    ...
    expect((chat2.find('[data-testid="chat-model-select"]').element as HTMLSelectElement).value).toBe(
      'moonshot-v1-8k',
    );
    chat2.unmount();
  });
