...
function logOf(id: number, overrides: Partial<MealLog> = {}): MealLog {
  return {
    id, logDate: '2026-08-10', dishId: null, dishSlug: null, title: `记录${id}`,
    mealSlot: 'DINNER', rating: null, note: '', authorId: 1, authorName: '站长',
    createdAt: '2026-08-10T12:00:00Z', ...overrides,
  }
}
...
beforeEach(() => {
  ...
  mockFetchDailyMenu.mockReset().mockResolvedValue({
    exists: true, date: todayISO(), status: 'DRAFT', note: '', version: 0,
    items: [{ id: 1, dishId: 9, dishSlug: 'mapo-tofu', title: '麻婆豆腐', mealSlot: 'DINNER', note: '', sortOrder: 0, authorId: 2, authorName: '小伙伴', createdAt: '2026-07-27T10:00:00Z' }],
    updatedBy: 1, updatedAt: null,
  })
...
describe('FD-17 时光机', () => {
  ...
  it('按日期分组渲染，含餐次/署名/星级/感想', async () => {
    mockFetchMealLogs.mockResolvedValue(logPage([
      logOf(1, { logDate: '2026-08-10', title: '麻婆豆腐', rating: 5, note: '今天特别嫩', authorName: '小伙伴' }),
  ...
  it('多页时提供"加载更早"，点击取下一页并追加', async () => {
    mockFetchMealLogs.mockResolvedValueOnce(logPage([logOf(1)], 2))
      .mockResolvedValueOnce({ items: [logOf(2, { logDate: '2026-08-01' })], page: 1, size: 20, totalElements: 2, totalPages: 2 })
    const wrapper = await mountTimeline()
...
describe('FD-18 一键打卡入口', () => {
  it('菜单卡 ✓ 以菜单项形状打卡并弹提示', async () => {
    ...
    expect(mockCreateMealLog).toHaveBeenCalledWith({
      dishSlug: 'mapo-tofu', mealSlot: 'DINNER', logDate: todayISO(),
    })
