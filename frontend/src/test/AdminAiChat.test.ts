...
function streamResolve(content: string) {
  mockStreamAiChat.mockImplementation(async (_messages: unknown, callbacks: adminApi.AiStreamCallbacks) => {
...
describe('AdminAiChat Component', () => {
  ...
  it('switches provider and resets the model to that provider default', async () => {
    ...
    expect((wrapper.find('[data-testid="chat-model-select"]').element as HTMLSelectElement).value).toBe(
      'b-2',
    );
  ...
  it('refreshes stale provider models after the provider registry changes', async () => {
    ...
    expect(
      wrapper.findAll('[data-testid="chat-model-select"] option').map((option) => option.text()),
    ).toEqual(['new-model']);
  });
  ...
  it('重挂载后保持用户已选的模型（与宠物面板/供应商页同源）', async () => {
    ...
    await flushPromises();
    expect((wrapper2.find('[data-testid="chat-model-select"]').element as HTMLSelectElement).value).toBe(
      'qwen3.7-plus',
    );
    wrapper2.unmount();
  });
  ...
  it('displays loading state, disables inputs, then renders streamed deltas', async () => {
    ...
    mockStreamAiChat.mockImplementation((_messages: unknown, callbacks: adminApi.AiStreamCallbacks) => {
  ...
  it('clears conversation when confirmed', async () => {
    window.sessionStorage.setItem(
      'yubai-admin-ai-messages',
      JSON.stringify([{ role: 'user', content: 'Message to clear' }]),
    );
...
describe('FD-29 宠物动画事件（供 AdminPetAssistant 驱动状态，不改请求逻辑）', () => {
  ...
  it('展示后端返回的安全错误消息（不吞错、不透传供应商原始响应）', async () => {
    mockStreamAiChat.mockRejectedValue(
      new adminApi.AiStreamHttpError(502, 'OpenCode Server returned an error'),
    );
...
describe('AI 聊天历史侧边栏', () => {
  ...
  it('首次发送创建会话，流式结束后把问答写入历史', async () => {
    ...
    expect(mockCreateAiChatSession).toHaveBeenCalledTimes(1);
    expect(mockAppendAiChatMessages).toHaveBeenCalledWith(1, [
  ...
  it('确认后删除会话记录并从列表移除', async () => {
    mockFetchAiChatSessions.mockResolvedValueOnce([SESSION_1, SESSION_2]).mockResolvedValue([SESSION_2]);
    vi.spyOn(window, 'confirm').mockReturnValue(true);
