...
describe('streamAiChat SSE 解析器', () => {
  ...
  it('error 事件抛出 AiStreamHttpError 并取消底层 reader', async () => {
    ...
    const deltas: string[] = [];
    await expect(streamAiChat(MESSAGES, { onDelta: (t) => deltas.push(t) })).rejects.toMatchObject({
      name: 'AiStreamHttpError',
      status: 502,
    });
  ...
  it('非 2xx 响应解析 JSON message 并抛出对应状态码', async () => {
    ...
    await expect(streamAiChat(MESSAGES, { onDelta: () => {} })).rejects.toMatchObject({
      status: 429,
      message: '请求过于频繁',
    });
  });
  ...
  it('401 清空登录态并抛出', async () => {
    ...
    await expect(streamAiChat(MESSAGES, { onDelta: () => {} })).rejects.toBeInstanceOf(AiStreamHttpError);
