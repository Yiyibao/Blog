...
beforeEach(() => {
  ...
  mockFetchDishes.mockResolvedValue(
    pageOf([makeDish(), makeDish({ id: 2, slug: 'plain-congee', name: '白粥', favoriteCount: 0 })]),
  );
  mockFetchDishFavorites.mockResolvedValue(
    pageOf([favoriteItem('mapo-tofu', '麻婆豆腐', 9), favoriteItem('plain-congee', '白粥', 2)]),
  );
});
...
describe('FoodSection favorites integration', () => {
  ...
  it('optimistically bumps the count then applies the server value on favorite', async () => {
    ...
    mockFavoriteDish.mockReturnValue(
      new Promise((r) => {
        resolve = r;
      }),
    );
