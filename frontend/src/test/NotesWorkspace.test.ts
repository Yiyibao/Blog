...
const routerMock = vi.hoisted(() => {
  ...
  return {
    ...
    get leaveGuard() { return leaveGuard },
    set leaveGuard(fn) { leaveGuard = fn },
  }
})
...
vi.mock('vue-router', () => ({
  ...
  onBeforeRouteLeave: (fn: any) => { routerMock.leaveGuard = fn },
}))
...
const adminApi = vi.hoisted(() => {
  ...
  function mock(name: string) {
    queues[name] = []
    return vi.fn(() => new Promise<any>((resolve, reject) => {
      queues[name]!.push({ resolve, reject })
    }))
  }

  return {
    requests(name: string) { return queues[name] ?? [] },
    resolve(name: string, value: any) { const q = queues[name]; if (q && q.length) q.shift()!.resolve(value) },
...
const S1: AdminNote = {
  id: 1, title: 'A', markdownContent: '# A\nHello', folder: 'w',
  status: 'DRAFT', tags: [], version: 1, wordCount: 3,
...
const ATTACHMENT: NoteAttachment = {
  id: 10, publicId: 'p1', noteId: 1, fileName: 'i.png',
  mediaType: 'image/png', byteSize: 100,
  url: canonicalUrl, createdAt: '',
}
...
describe('NotesWorkspace regression', () => {
  ...
  it('5a: upload in progress blocks note switching', async () => {
    ...
    adminApi.uploadNoteAttachment.mockImplementationOnce(
      () => new Promise(() => {})
    )
  ...
  it('5b: after upload completes, note switching works', async () => {
    ...
    adminApi.fetchNoteAttachmentContent.mockImplementationOnce(
      () => Promise.resolve(new Blob(['d']))
    )
    w.findComponent(FakeTyporaEditor).vm.simulateUpload()
    await flush()
