...
describe('seoBlock', () => {
  ...
  it('includes JSON-LD when provided', () => {
    const block = mod.seoBlock({ title: 'Home', jsonLd: { '@context': 'https://schema.org', '@type': 'WebSite' } })
...
describe('generateDynamicArticleRoutes', () => {
  const mockPosts = {
    ...
    data: {
      items: [
        { slug: 'test-post', title: 'Test Post', excerpt: '<p>Test excerpt</p>', date: '2026-07-01', tags: ['test'], status: 'PUBLISHED' },
        { slug: 'draft-post', title: 'Draft', excerpt: 'Draft', date: '2026-07-02', status: 'DRAFT' },
        { slug: 'another', title: 'Another Post', excerpt: 'Clean excerpt', date: '2026-06-15', tags: ['dev'] },
      ],
      page: 0,
      size: 50,
      totalElements: 2,
  ...
  it('rejects HTTP errors from API', async () => {
    ...
    await expect(mod.generateDynamicArticleRoutes(TEMPLATE, 'https://test.example.com/api/v1'))
...
describe('generateDynamicSeriesRoutes', () => {
  const mockSeries = {
    ...
    message: 'success',
    data: [
      { slug: 'vue-guide', name: 'Vue Guide', description: 'A series about Vue.js', entryCount: 5, publishedAt: '2026-01-01T00:00:00Z' },
      { slug: 'design-patterns', name: 'Design Patterns', description: 'Common design patterns', entryCount: 3, publishedAt: '2026-02-01T00:00:00Z' },
    ],
  }
...
describe('prerender', () => {
  ...
  it('generates static and noindex routes without API base', async () => {
    ...
    expect(result.static.length).toBe(3)
  ...
  it('optionally fetches dynamic routes when apiBase is provided', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue({
      ...
      json: () => Promise.resolve({
        code: 200,
        message: 'success',
        data: { items: [{ slug: 'integrated-post', title: 'Integrated', excerpt: 'Test', date: '2026-07-01' }], page: 0, size: 50, totalElements: 1, totalPages: 1 },
      }),
    }))
    ...
    fetchMock.mockImplementationOnce(() => Promise.resolve({
      ok: true,
      json: () => Promise.resolve({
        code: 200, message: 'success', data: { items: [{ slug: 'integrated-post', title: 'Integrated', excerpt: 'Test', date: '2026-07-01' }], page: 0, size: 50, totalElements: 1, totalPages: 1 },
      }),
    }))
    fetchMock.mockImplementationOnce(() => Promise.resolve({
      ok: true,
