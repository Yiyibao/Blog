...
import {
  initBrowserSession, connectOnce, createPageTarget, killResidualBrowsers, cleanupStaleProfiles,
  assertOwnedProfileDir, extractUserDataDir, isAllowedBrowserProcess,
  inspectOwnedProfile, pathEquals, waitCdpReady, waitChildExit, summarizeStderr, classifyExit,
  PROFILE_PREFIX, MAX_INIT_RETRIES,
  DEFAULT_EDGE_ARGS,
...
class MockWebSocket {
  ...
  send(data: string) {
    ...
    const result = message.method === 'Target.attachToTarget'
      ? { sessionId: `SESSION-${this.instanceIndex + 1}` }
      : {}
...
function connError(code: string) {
  return Object.assign(new Error(`connect ${code} 127.0.0.1:9400`), {
    code, errno: -4078, syscall: 'connect', address: '127.0.0.1', port: 9400,
  })
}
...
function mockLocalHttp(state: LocalState) {
  return async ({ port, path }: { port: number; path: string }): Promise<LocalHttpResponse> => {
    ...
    if (path.startsWith('/json/version')) {
      ...
      if ((state.mode === 'version-then-dead' || state.mode === 'new-500-then-dead') && state.versionCalls > 4) {
    ...
    if (path.startsWith('/json/new')) {
      ...
      if (mode === 'all-not-page') return jsonResponse({ id: 'T-1', type: 'iframe', webSocketDebuggerUrl: 'ws://x' })
...
function makeDeps(overrides: Partial<BrowserSessionDeps> = {}): BrowserSessionDeps {
  ...
  return {
    ...
    WebSocket: (url: string) => new MockWebSocket(url) as unknown as ReturnType<BrowserSessionDeps['WebSocket']>,
    ...rest,
  }
}
...
describe('P1 会话初始化与重试', () => {
  ...
  it('正常初始化：owner.json 写入、Page.enable/Runtime.enable 成功、cleanup 一次', async () => {
    ...
    const deps = makeDeps({
      spawn: () => { const c = createMockChild(); children.push(c); return c },
    ...
    const session = await initBrowserSession({
      deps, browserPath: 'C:/mock/msedge.exe', profileDir: profileDir('test-ok'), maxRetries: 2,
    })
  ...
  it('Page.enable 失败后整轮重试：第二轮使用新 socket，第一次 socket 已关闭', async () => {
      ...
      if (ws.instanceIndex === 0) {
        ws.failOnSend = (msg) => {
          if (msg.method === 'Page.enable') {
            ...
            ws.emit('close', Object.assign(new Event('close'), { code: 1006, reason: 'abnormal', wasClean: false }))
          }
        }
      }
    }
    ...
    const session = await initBrowserSession({
      deps, browserPath: 'C:/mock/msedge.exe', profileDir: profileDir('test-retry'), maxRetries: 3,
    })
  ...
  it('重试耗尽后抛出非零错误，且每次重试都启动新浏览器', async () => {
    ...
    await expect(initBrowserSession({
      deps, browserPath: 'C:/mock/msedge.exe', profileDir: profileDir('test-exhaust'), maxRetries: 2,
  ...
  it('浏览器在 CDP 就绪前提前退出：明确报出退出码', async () => {
    ...
    await expect(connectOnce({
      deps, browserPath: 'C:/mock/msedge.exe', profileDir: profileDir('test-exit'), cdpPort: 9999,
  ...
  it('version 成功后 child 退出：报出退出码（时间线证据）', async () => {
    ...
    await expect(connectOnce({
      deps, browserPath: 'C:/mock/msedge.exe', profileDir: profileDir('test-version-exit'), cdpPort: 9998,
    })).rejects.toThrow('PROCESS_EXITED exitCode=1')
  ...
  it('CDP 监听 PID 为合法后代（使用同 profile）时允许连接', async () => {
    ...
    const deps = makeDeps({
      ...
      listBrowserProcesses: async () => [
        { pid: 7777, commandLine: `"C:\\Program Files (x86)\\Microsoft\\Edge\\Application\\msedge.exe" --user-data-dir="${attemptProfile}"` },
      ],
    })
    const session = await connectOnce({
      deps, browserPath: 'C:/mock/msedge.exe', profileDir: attemptProfile, cdpPort: 9997,
    })
    expect(session.targetId).toBe('TARGET-1')
  })
  ...
  it('CDP 监听 PID 被无关进程抢占：拒绝连接（TOCTOU 防护）', async () => {
    ...
    const deps = makeDeps({
      ...
      listBrowserProcesses: async () => [
        { pid: 8888, commandLine: '"C:\\Program Files (x86)\\Microsoft\\Edge\\Application\\msedge.exe" --user-data-dir="C:\\Users\\me\\AppData\\Local\\Edge"' },
      ],
      localHttpRequest: mockLocalHttp(createState()),
    })
    await expect(connectOnce({
      deps, browserPath: 'C:/mock/msedge.exe', profileDir: profileDir('test-touctou'), cdpPort: 9996,
  ...
  it('target 建连失败（target 失效）：两次建连尝试后失败并整体重试', async () => {
    ...
    const session = await initBrowserSession({
      deps, browserPath: 'C:/mock/msedge.exe', profileDir: profileDir('test-target-gone'), maxRetries: 3,
    })
    expect(session.targetId).toBe('TARGET-2')
    expect(children).toHaveLength(2)
  })
...
describe('P1 /json/new page target 创建与诊断（node:http）', () => {
  ...
  it('首次调用失败、第二次成功（first-call-fail）', async () => {
    ...
    const session = await initBrowserSession({
      deps: captureDeps(state), browserPath: 'C:/mock/msedge.exe',
      profileDir: profileDir('test-new-retry'), maxRetries: 2,
    })
    expect(session.targetId).toBe('TARGET-1')
  ...
  it('重试耗尽：最终错误汇总每轮失败摘要', async () => {
    ...
    await expect(initBrowserSession({
      deps: captureDeps(state), browserPath: 'C:/mock/msedge.exe',
      profileDir: profileDir('test-new-exhaust'), maxRetries: 2,
...
describe('P1 每 attempt 全新 profile/动态端口/进程/target/socket', () => {
  it('三次失败 attempt 的 profile、child PID、target、socket 全部不同，端口由 DevToolsActivePort 提供，且上一轮 profile 已安全清理', async () => {
    ...
    const deps = makeDeps({
      ...
      removeDir: async (dir: string) => { removedDirs.push(dir) },
    })
    await expect(initBrowserSession({
      deps, browserPath: 'C:/mock/msedge.exe',
      profileDir: profileDir('test-attempts'), maxRetries: 3,
    })).rejects.toThrow('重试耗尽')
...
describe('P1 inspectOwnedProfile 与 cleanupStaleProfiles 活跃保护', () => {
  ...
  it('活跃 owner 的 profile 不删除；owner 已退出且无浏览器使用才删除', async () => {
    ...
    const deps = makeDeps({
      ...
      listBrowserProcesses: async () => [],
      removeDir: async (dir: string) => { removed.push(dir) },
  ...
  it('owner 已退出但浏览器仍使用该 profile：不删除', async () => {
    ...
    const deps = makeDeps({
      ...
      isPidAlive: async () => false,
      listBrowserProcesses: async () => [
        { pid: 55, commandLine: `"C:\\Program Files (x86)\\Microsoft\\Edge\\Application\\msedge.exe" --user-data-dir="${activeDir}"` },
      ],
      removeDir: async (dir: string) => { removed.push(dir) },
    ...
    const deps = makeDeps({
      ...
      listBrowserProcesses: async () => {
        ...
        return inspections >= 2
          ? [{ pid: 66, commandLine: `"C:\\Program Files (x86)\\Microsoft\\Edge\\Application\\msedge.exe" --user-data-dir="${activeDir}"` }]
          : []
      },
      removeDir: async (dir: string) => { removed.push(dir) },
      listProfileCandidates: async () => [activeDir],
  ...
  it('进程枚举失败时不删除（安全失败）', async () => {
    ...
    const deps = makeDeps({
      readOwnerFile: async () => ({ pid: 0 }),
      isPidAlive: async () => false,
      listBrowserProcesses: async () => { throw new Error('CIM down') },
      removeDir: async (dir: string) => { removed.push(dir) },
      listProfileCandidates: async () => [activeDir],
    })
    expect(await cleanupStaleProfiles(deps)).toBe(0)
  ...
  it('并发两个会话互不影响：两个 owner 均存活时全部保留且进程不终止', async () => {
    ...
    const deps = makeDeps({
      listBrowserProcesses: async () => [
        { pid: 71, commandLine: `"C:\\Program Files (x86)\\Microsoft\\Edge\\Application\\msedge.exe" --user-data-dir="${activeDir}"` },
        { pid: 72, commandLine: `"C:\\Program Files (x86)\\Microsoft\\Edge\\Application\\msedge.exe" --user-data-dir="${staleDir}"` },
      ],
      listProfileCandidates: async () => [activeDir, staleDir],
      readOwnerFile: async (dir: string) => ({ pid: dir === activeDir ? 100 : 200 }),
      isPidAlive: async () => true,
      killProcessTree: async (pid: number) => { killed.push(pid) },
      removeDir: async (dir: string) => { removed.push(dir) },
    })
  ...
  it('大小写变体的 user-data-dir 匹配同 profile（win32，不删除）', async (context) => {
    ...
    const deps = makeDeps({
      ...
      isPidAlive: async () => false,
      listBrowserProcesses: async () => [
        { pid: 77, commandLine: `"C:\\Program Files (x86)\\Microsoft\\Edge\\Application\\msedge.exe" --user-data-dir="${variant}"` },
      ],
      removeDir: async (dir: string) => { removed.push(dir) },
  ...
  it('inspectOwnedProfile 四态（路径非法/归属不明/存活/过期）', async () => {
    ...
    expect((await inspectOwnedProfile(join(tmpRoot, 'Temp-sibling', PROFILE_PREFIX + 'x'), deps)).pathValid).toBe(false)
...
describe('P1 waitCdpReady readiness gate', () => {
  ...
  function readyDeps({
    ...
    return makeDeps({
      localHttpRequest: async ({ path }: { path: string }) => {
        if (path.startsWith('/json/version')) {
          versionCalls += 1
          if (versionCalls <= versionCallsToFail) throw Object.assign(new Error('read ECONNRESET'), { code: 'ECONNRESET' })
          return { status: 200, contentType: 'application/json', text: '{}', json: { Browser: browserCalls(versionCalls) } }
        }
  ...
  it('listenerPid=null 时不得进入就绪（持续不稳定 → 超时失败）', async () => {
    ...
    await expect(waitCdpReady({
      deps, cdpPort: 9400, child, ownedProfile: profile,
      browserExitedRef: () => null, timeoutMs: 2000, stableProbes: 3,
  ...
  it('连续 3 次稳定探测（Browser 一致、listener 一致、child 存活）通过', async () => {
    ...
    const result = await waitCdpReady({
      deps, cdpPort: 9400, child, ownedProfile: profile,
      browserExitedRef: () => null, timeoutMs: 5000, stableProbes: 3,
    })
  ...
  it('child 提前退出（实时 getter）：立即失败并报退出码', async () => {
    ...
    const promise = waitCdpReady({
      deps, cdpPort: 9400, child, ownedProfile: profile,
      browserExitedRef: () => exited, timeoutMs: 5000, stableProbes: 3,
    })
  ...
  it('listener 转移到合法后代（使用同 profile）时通过并报告', async () => {
    ...
    const deps = makeDeps({
      ...
      listBrowserProcesses: async () => [
        { pid: 7777, commandLine: `"C:\\Program Files (x86)\\Microsoft\\Edge\\Application\\msedge.exe" --user-data-dir="${profile}"` },
      ],
    })
    const result = await waitCdpReady({
      deps, cdpPort: 9400, child, ownedProfile: profile,
...
describe('P1 崩溃分类与进程树生命周期', () => {
  ...
  it('GPU 子进程 0xC0000022 分类', () => {
    const result = classifyExit({ code: null, stderr: 'GPU process exited unexpectedly: exit_code=-1073741790' })
  ...
  it('GPUPersistentCache 占用日志分类', () => {
    const result = classifyExit({ code: null, stderr: 'Failed to open persistent cache files in: C:\\Temp\\pet-layout-check-1\\GPUPersistentCache\\DawnGraphiteCache\\... 另一个程序正在使用此文件，进程无法访问 (0x20)' })
  ...
  it('waitChildExit 等待使用同 profile 的后代进程退出后才返回', async () => {
    ...
    let descendants = [{ pid: 5000, commandLine: `"C:\\Program Files (x86)\\Microsoft\\Edge\\Application\\msedge.exe" --user-data-dir="${profile}"` }]
    let listCalls = 0
    const deps = makeDeps({
      listBrowserProcesses: async () => {
        listCalls += 1
  ...
  it('waitChildExit 在后代迟迟不退时走 taskkill 兜底', async () => {
    ...
    const deps = makeDeps({
      listBrowserProcesses: async () => [
        { pid: 5001, commandLine: `"C:\\Program Files (x86)\\Microsoft\\Edge\\Application\\msedge.exe" --user-data-dir="${profile}"` },
      ],
      killProcessTree: async () => {},
      readOwnerFile: async () => ({ pid: 0 }),
      isPidAlive: async () => false,
  ...
  it('原始 child 退出但合法后代仍存活：readiness 继续而非崩溃', async () => {
    ...
    const deps = makeDeps({
      ...
      listBrowserProcesses: async () => [
        { pid: 5002, commandLine: `"C:\\Program Files (x86)\\Microsoft\\Edge\\Application\\msedge.exe" --user-data-dir="${profile}"` },
      ],
      readOwnerFile: async () => ({ pid: 0 }),
      isPidAlive: async () => false,
    })
  ...
  it('原始 child 与全部后代均退出：判定崩溃并报退出码', async () => {
    ...
    await expect(waitCdpReady({
      deps, cdpPort: 9400, child, ownedProfile: profile,
      browserExitedRef: () => exited, timeoutMs: 2000, stableProbes: 3,
...
describe('P1 killResidualBrowsers 所有权与过期判定', () => {
  ...
  it('用户浏览器（无脚本前缀）不杀', async () => {
    ...
    const deps = makeDeps({
      listBrowserProcesses: async () => [
        { pid: 11, commandLine: '"C:\\Program Files (x86)\\Microsoft\\Edge\\Application\\msedge.exe" --user-data-dir="C:\\Users\\me\\AppData\\Local\\Edge" about:blank' },
      ],
      killProcessTree: async (pid: number) => { killed.push(pid) },
    })
    expect(await killResidualBrowsers(deps)).toBe(0)
  ...
  it('命令行仅在无关参数中含前缀不杀', async () => {
    ...
    const deps = makeDeps({
      listBrowserProcesses: async () => [
        { pid: 41, commandLine: '"C:\\Program Files (x86)\\Microsoft\\Edge\\Application\\msedge.exe" --foo=pet-layout-check-x about:blank' },
      ],
      killProcessTree: async (pid: number) => { killed.push(pid) },
    })
    expect(await killResidualBrowsers(deps)).toBe(0)
  ...
  it('畸形或恶意 user-data-dir 不杀', async () => {
    ...
    const deps = makeDeps({
      listBrowserProcesses: async () => [
        { pid: 51, commandLine: '"C:\\Program Files (x86)\\Microsoft\\Edge\\Application\\msedge.exe" --user-data-dir="C:\\Users\\me\\pet-layout-check-x"' },
        { pid: 52, commandLine: '"C:\\Program Files (x86)\\Microsoft\\Edge\\Application\\msedge.exe" --user-data-dir="C:\\Temp-sibling\\pet-layout-check-danger"' },
      ],
  ...
  it('非 Edge/Chrome 可执行文件不杀', async () => {
    ...
    const deps = makeDeps({
      listBrowserProcesses: async () => [
        { pid: 61, commandLine: 'notepad.exe --user-data-dir="C:\\Users\\Hfff\\AppData\\Local\\Temp\\pet-layout-check-x"' },
      ],
      killProcessTree: async (pid: number) => { killed.push(pid) },
    })
    expect(await killResidualBrowsers(deps)).toBe(0)
  ...
  it('CIM 查询失败时安全失败（返回 0，不扩大清理）', async () => {
    const deps = makeDeps({
      listBrowserProcesses: async () => { throw new Error('CIM down') },
      killProcessTree: async () => { throw new Error('should not be called') },
    })
    expect(await killResidualBrowsers(deps)).toBe(0)
  })

  it('extractUserDataDir 解析引号与非引号形式', () => {
    expect(extractUserDataDir('msedge.exe --user-data-dir="C:\\Temp\\pet-layout-check-a"')).toBe('C:\\Temp\\pet-layout-check-a')
    expect(extractUserDataDir('msedge.exe --user-data-dir=C:\\Temp\\pet-layout-check-b')).toBe('C:\\Temp\\pet-layout-check-b')
    expect(extractUserDataDir('msedge.exe about:blank')).toBeNull()
  })

  it('isAllowedBrowserProcess 只认 msedge/chrome（引号内空格安全）', () => {
    expect(isAllowedBrowserProcess('"C:\\Program Files (x86)\\Microsoft\\Edge\\Application\\msedge.exe" --headless')).toBe(true)
