...
describe('P1 深色主题：聊天区域 token 化契约', () => {
  ...
  it('关键区域全部改用项目主题 token', () => {
    ...
    expect(style).toMatch(/\.chat-textarea:focus \{[\s\S]*?box-shadow: 0 0 0 3px color-mix\(in srgb, var\(--accent,/)
    expect(style).toMatch(/\.chat-textarea:focus \{[\s\S]*?outline: 2px solid color-mix\(in srgb, var\(--accent,/)
  })

  it('用到的 token 在 tokens.css 的 :root.dark 中都有定义（深色可计算）', () => {
    const darkTokens = extractTokenValues(TOKENS_CSS, true)
    for (const token of ['--surface', '--surface-solid', '--ink', '--muted', '--line', '--line-strong', '--accent', '--paper']) {
