...
describe('FD-12 foodStore 菜单分片', () => {
  ...
  it('对方在上次看过之后加的菜被标为 arrivals（到达动画数据源）', async () => {
    ...
    mockFetchDailyMenu.mockResolvedValue(
      menuOf([
  ...
  it('append 乐观插入临时项，成功后以服务端响应整单替换', async () => {
    ...
    mockAppendMenuItem.mockReturnValue(
      new Promise((r) => {
        resolve = r;
      }),
    );
  ...
  it('append 失败回滚乐观项并抛分类错误', async () => {
    ...
    mockFetchDailyMenu.mockResolvedValue(menuOf([{}]));
    await store.loadMenu();
    mockAppendMenuItem.mockRejectedValue(
      Object.assign(new Error('429'), {
        isAxiosError: true,
        response: { status: 429, data: {}, headers: {} },
      }),
    );
    await expect(store.append({ title: '手速菜', mealSlot: 'DINNER' })).rejects.toMatchObject({
      kind: 'rate-limited',
    });
    expect(store.menu?.items).toHaveLength(1);
  });
  ...
  it('submitMenu 遇 409 自动刷新最新菜单再抛冲突', async () => {
    ...
    await expect(
      store.submitMenu({ status: 'CONFIRMED', note: '', expectedVersion: 0, items: [] }),
  ...
  it('切日期期间的迟到响应被丢弃（竞态守卫）', async () => {
    ...
    mockFetchDailyMenu.mockReturnValueOnce(
      new Promise((r) => {
        resolveSlow = r;
      }),
    );
  ...
  it('stale same-date read discarded after mutation (generation guard)', async () => {
    ...
    mockAppendMenuItem.mockResolvedValue(
      menuOf([
        { id: 1, title: '初始菜' },
        { id: 2, title: '新加的菜' },
      ]),
