...
import {
  createCdpClient, describeConnectionFailure, describePostOpenFailure,
  redactWsUrl, DISCONNECT_SETTLE_MS, DEFAULT_SEND_TIMEOUT_MS,
...
describe('createCdpClient', () => {
  ...
  it('请求超时（15s 有界）且错误信息包含 method', async () => {
    ...
    const assertion = expect(promise).rejects.toThrow(`CDP send 超时（${DEFAULT_SEND_TIMEOUT_MS}ms）: Page.navigate`)
