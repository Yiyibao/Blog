...
vi.mock('axios', async (importOriginal) => {
  ...
  const instance = {
    interceptors: {
      request: {
        use: (fulfilled: typeof http.requestInterceptor) => {
          http.requestInterceptor = fulfilled;
        },
      },
      response: {
        use: (_fulfilled: unknown, rejected: typeof http.responseRejected) => {
          http.responseRejected = rejected;
        },
      },
    },
    request,
    get: (url: string, config: Record<string, any> = {}) =>
      request({ ...config, url, method: 'get', headers: config.headers ?? {} }),
    post: (url: string, data?: unknown, config: Record<string, any> = {}) =>
      request({ ...config, url, data, method: 'post', headers: config.headers ?? {} }),
    put: (url: string, data?: unknown, config: Record<string, any> = {}) =>
      request({ ...config, url, data, method: 'put', headers: config.headers ?? {} }),
    delete: (url: string, config: Record<string, any> = {}) =>
      request({ ...config, url, method: 'delete', headers: config.headers ?? {} }),
  };
  return {
    ...actual,
    default: {
...
describe('6C-1 refresh flow', () => {
  it('returns a TOTP challenge from a 202 login response', async () => {
    ...
    await expect(admin.login('admin', 'secret', { challengeId: 'human', nonce: '0' })).resolves.toEqual({
      totpRequired: true,
  ...
  it('does not refresh after a login 401', async () => {
    ...
    await expect(admin.login('bad', 'bad', { challengeId: 'id', nonce: '0' })).rejects.toMatchObject({
      isAxiosError: true,
    });
