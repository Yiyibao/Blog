...
export function seoBlock(params) {
  ...
  if (jsonLd)
    h += `${ind}<script type="application/ld+json" data-structured>${safeJsonLd(jsonLd)}</script>\n`;
  return h;
}
...
const STATIC_ROUTES_CONFIG = [
  ...
  { path: '/about', title: '关于我', description: '了解这个网站与它记录的内容。', ogType: 'website' },
  { path: '/categories', title: '文章分类', description: '按主题浏览已发布的文章分类。', ogType: 'website' },
...
export async function generateDynamicSeriesRoutes(template, apiBase) {
  ...
  if (data.length > MAX_DYNAMIC_PAGES)
    throw new Error(`Series count exceeds prerender limit ${MAX_DYNAMIC_PAGES}`);
  const results = [];
...
export async function generateDynamicTagRoutes(template, apiBase) {
  ...
  if (data.length > MAX_DYNAMIC_PAGES)
    throw new Error(`Tag count exceeds prerender limit ${MAX_DYNAMIC_PAGES}`);
  const results = [];
